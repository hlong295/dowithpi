...
function getSandboxFlag(): boolean {
  // Default to sandbox=true unless explicitly disabled.
  // This matches Pi testnet/dev flows. When moving to mainnet, set:
  //   NEXT_PUBLIC_PI_SANDBOX=false
  const env = (process.env.NEXT_PUBLIC_PI_SANDBOX ?? "").toLowerCase()
  if (env === "false" || env === "0" || env === "no") return false

  const net = (process.env.NEXT_PUBLIC_PI_NETWORK ?? "").toLowerCase()
  if (net === "mainnet") return false
  if (net === "testnet") return true

  // If not specified, keep sandbox on (safer default for development/testnet).
  return true
}
...
> {
  ...
  try {
    if (!_piInitDone && typeof Pi.init === "function") {
      ...
      Pi.init({ version: "2.0", sandbox: getSandboxFlag() })
      _piInitDone = true
    }
  } catch (e) {
...
export async function signInWithPi(scopes: string[] = ["username"]): Promise<PiAuthResult> {
  ...
  try {
    // Pi Browser / Pi SDK đôi khi phản hồi chậm (đặc biệt khi mạng yếu).
    // Không nên timeout quá ngắn vì sẽ gây lỗi "timeout" giả.
    ...
    const res = await withTimeout(
      authPromise,
      120000, // 120s timeout để tránh Pi Browser bị chậm
      "PI_AUTH_TIMEOUT",
    )
