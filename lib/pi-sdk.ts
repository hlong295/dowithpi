...
> {
  ...
  try {
    if (!_piInitDone && typeof Pi.init === "function") {
      // IMPORTANT: Keep the existing login flow intact; only make sandbox follow env.
      // In Testnet/Pi Browser test builds, sandbox=true is commonly required.
      const sandboxEnv = String(process.env.NEXT_PUBLIC_PI_SANDBOX ?? "").toLowerCase()
      const sandbox = sandboxEnv === "true" || sandboxEnv === "1" || sandboxEnv === "yes"
...
export async function signInWithPi(
  ...
  try {
    // Wrap authenticate call with a generous timeout.
    // Pi Browser / SDK auth can take longer on mobile networks; using a short
    // timeout causes false "Pi Network bị timeout" even though Pi SDK is connected.
    const authPromise = Pi.authenticate(scopes, (payment: any) => {
      console.log("[pi-sdk] onIncompletePaymentFound:", payment)
    })

    const res = await withTimeout(authPromise, timeoutMs, "PI_AUTH_TIMEOUT")
