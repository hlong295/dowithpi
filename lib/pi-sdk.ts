...
> {
  ...
  try {
    if (!_piInitDone && typeof Pi.init === "function") {
      // IMPORTANT:
      // Pi Browser can run in Sandbox/Testnet or Mainnet.
      // If we init the SDK with the wrong sandbox flag, payments can fail/timeout.
      // Control it via NEXT_PUBLIC_PI_SANDBOX.
