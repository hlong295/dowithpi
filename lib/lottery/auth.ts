...
// ---------------------------------------------------------------------------
// Lightweight auth-kind inference used by lottery registration gating.
// We must NOT change login flow; this only classifies the current request.

export type AuthKind = "pi" | "email" | "unknown";

export async function inferAuthKind(args: {
  requesterName: string | null;
  piUserIdHeader: string | null;
  masterUserId: string;
}): Promise<AuthKind> {
  const piId = String(args.piUserIdHeader || "").trim();
  if (piId) return "pi";

  const name = String(args.requesterName || "").trim();
  // If we have a Pi username (even without a Pi user id header), treat it as Pi.
  // This is needed for Pi App Studio environments where custom headers can be flaky.
  if (name && !name.includes("@")) return "pi";

  // Fallback: if there is a pi_users row for this master user id, treat as Pi.
  ...
  // Otherwise, assume email session.
  return "email";
}
