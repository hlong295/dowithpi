...
export async function getProductById(productId: string) {
  ...
  // - Prefer product.merchant_username/provider_name if present
  // - Fallback to pi_users (by provider_id) so UI shows the correct account that posted the product
  let resolvedProviderName: string =
    (data as any).merchant_username || (data as any).provider_name || ""

  if (!resolvedProviderName) {
    const providerId = (data as any).provider_id || (data as any).providerId || (data as any).merchant_id || (data as any).user_id
    if (providerId) {
      ...
      resolvedProviderName = providerRow?.provider_business_name || providerRow?.pi_username || ""
    }
  }

  if (!resolvedProviderName) resolvedProviderName = "PITODO"
