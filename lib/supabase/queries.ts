...
// Avoid heavy console logging in Pi Browser production.
const __dbgEnabled = () => {
  try {
...
export async function createOrUpdatePiUser(userData: { piUid: string; piUsername: string; accessToken: string }) {
  dlog("[v0] createOrUpdatePiUser: Calling API with:", {
  ...
  dlog("[v0] createOrUpdatePiUser: API response:", { status: response.status, data })
...
export async function getProductById(productId: string) {
  ...
  dlog("[v0] Product detail images:", { id: data.id, name: data.name, allImages, videoUrl })
  ...
  // Resolve provider display name from pi_users using provider_id (avoid default "PITODO")
  let providerDisplayName = (data as any).provider_name || data.merchant_username || ""
  if ((data as any).provider_id) {
    const { data: pr } = await supabase
      .from("pi_users")
      .select("pi_username, provider_business_name")
      .eq("id", (data as any).provider_id)
      .maybeSingle()
    const resolved = (pr?.provider_business_name || pr?.pi_username || "").trim()
    if (resolved) providerDisplayName = resolved
  }
  providerDisplayName = (providerDisplayName || "").trim() || "PITODO"
  ...
  return {
    ...
    supportsPitd: true,
    pitdPaymentEnabled: (data as any).pitd_payment_enabled ?? true,
    directContactName: (data as any).direct_contact_name ?? undefined,
    directContactPhone: (data as any).direct_contact_phone ?? undefined,
    directContactAddress: (data as any).direct_contact_address ?? undefined,
    directContactNote: (data as any).direct_contact_note ?? undefined,
    images: allImages,
    videoUrl: videoUrl,
...
export async function createProduct(productData: any) {
  ...
  dlog("[v0] createProduct: Inserting product with data:", productData)
  ...
  const insertData = {
    ...
    // PITD payment mode (when disabled: show direct contact instead of PITD exchange)
    pitd_payment_enabled:
      productData.pitdPaymentEnabled === undefined || productData.pitdPaymentEnabled === null
        ? true
        : !!productData.pitdPaymentEnabled,
    direct_contact_name: productData.directContactName || null,
    direct_contact_phone: productData.directContactPhone || null,
    direct_contact_address: productData.directContactAddress || null,
    direct_contact_note: productData.directContactNote || null,
  ...
  dlog("[v0] createProduct: Mapped insert data:", insertData)
  ...
  dlog("[v0] createProduct: Success:", data)
  return data
}
