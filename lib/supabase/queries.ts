...
export async function createOrUpdatePiUser(userData: { piUid: string; piUsername: string; accessToken: string }) {
  ...
  // IMPORTANT:
  // In Pi Browser, network requests can occasionally hang (no resolve/reject),
  // which leaves the UI stuck on a loading spinner forever. Put a hard timeout.
  const controller = new AbortController()
  const timeoutMs = 15000
  const timeout = setTimeout(() => controller.abort(), timeoutMs)
  let response: Response
  try {
    response = await fetch("/api/auth/pi-callback", {
      method: "POST",
      headers: {
    ...
    throw new Error(isAbort ? `PI_AUTH_TIMEOUT_${timeoutMs}` : (err?.message || "PI_AUTH_REQUEST_FAILED"))
  } finally {
