...
export async function resolveMasterUserId(
  ...
  if (p0?.id) {
    ...
    // IMPORTANT: only insert columns that actually exist in public.users (avoid "column does not exist" failures).
    // Schema (per your inventory): id, user_type, pi_uid, pi_username, email, full_name, user_role,
    // verification_status, provider_approved, provider_approved_at, provider_approved_by, last_login_at, email_verified.
