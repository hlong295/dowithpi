...
// Pi Browser đôi khi có trường hợp request fetch bị "treo" (không resolve, không reject)
// khiến màn hình "Đang xác thực với Pi Network..." quay vô hạn.
// -> Bắt buộc bọc timeout cứng cho các request auth quan trọng.
async function fetchJsonWithTimeout(
  ...
  try {
    const res = await fetch(input, {
      ...
      // tránh cache gây kết quả cũ / treo
      cache: "no-store",
    });
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loginWithPi = async (): Promise<{ success: boolean; piUserId?: string }> => {
    try {
      ...
      // NOTE:
      // Login should ONLY request the minimal scope ("username").
      // Requesting "payments" at login time can cause Pi Browser to stall/timeout
      // on some environments (testnet/mainnet wallet readiness, approvals, etc.).
      // We will request "payments" only when initiating a payment flow.
      // Shorter timeout for login so UI doesn't spin forever in Pi Browser.
      const piAuthResult = await signInWithPi(["username"], 30000)
