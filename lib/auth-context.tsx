		...
		const normalizeUser = (raw: any): User | null => {
			...
			if (inferredType === "pi") {
				...
				return {
					...
					memberLabel: (raw.memberLabel ?? raw.member_label ?? "normal") as any,
          ...
          const email = session.user.email || undefined
					...
					console.log("[v0] AuthProvider: ensurePiUserRecord returned:", userData?.pi_username || "(fallback)")
						...
						setUser({
							...
							memberLabel: (userData as any)?.member_label === "trusted" ? "trusted" : "normal",
							providerLabel: ((userData as any)?.provider_label as any) || undefined,
						})
        }
      } catch (error) {
