...
// NOTE: Some helpers in this file run outside React component scope.
// Keep a module-scope logger so we don't accidentally reference a component-local `log`.
// Pi Browser often hides console logs; this is mainly for Web debugging.
const log = (...args: any[]) => {
  try {
    if (typeof window === "undefined") return
    const qs = new URLSearchParams(window.location.search)
    const debug = qs.get("dbg") === "1" || window.location.hostname === "localhost"
    if (!debug) return
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Use module-scope `log` to avoid accidental ReferenceError in helpers below.
    ...
    const initializeAuth = async () => {
      ...
      localStorage.removeItem(PI_USER_ID_STORAGE_KEY)
        ...
        if (session?.user?.email) {
          ...
          await ensureUserOnce(sUser.email, "email", sUser, session.access_token)
...
// Pi Studio/Pi Browser may proxy requests through a rate-limited endpoint,
// so concurrent calls can easily hit 429 and break JSON parsing.
//
// IMPORTANT:
// - Some callers pass (identifier, method) and optionally (sessionUser).
// - We must support BOTH Pi login (no Supabase session) and Email login.
// - Do not change UI/flow; only dedupe and harden.
// =========================================================
const __ensureUserInflight = new Map<string, Promise<any>>()

type EnsureMethod = "pi" | "email"

// Deduped, signature-compatible helper.
// - identifier: for Pi login => UUID (preferred) or pi_uid (legacy);
//               for Email login => Supabase auth user id OR email (fallback).
// - method: "pi" | "email"
// - sessionUser: optional Supabase session.user (when available)
async function ensureUserOnce(identifier?: string | null, method?: EnsureMethod, sessionUser?: any, accessToken?: string) {
  try {
    const ident = identifier ? String(identifier) : ""
    const m: EnsureMethod = method === "pi" ? "pi" : "email"
    if (!ident) return null

    const key = `${m}:${ident}`
    const existing = __ensureUserInflight.get(key)
    if (existing) return existing

    const p = (async () => {
      let u = sessionUser

      // If a caller accidentally passed ensureUserOnce(sessionUser) (old signature),
      // recover gracefully.
      if (!u && typeof identifier === "object") {
        u = identifier
      }

      // Normalize common shapes.
      if (u && typeof u === "object") {
        if (!u.id && (u.uid || u.user_id)) {
          u = { ...u, id: u.uid || u.user_id }
        }
      }

      // If still missing/invalid, build a minimal payload so ensure-user doesn't 400.
      if (!u || typeof u !== "object" || !u.id) {
        u = {
          id: ident,
          email: m === "email" ? ident : null,
          user_metadata: {
            from_pi: m === "pi",
            pi_uid: m === "pi" ? ident : null,
          },
        }
      }

      return await ensurePiUserRecord(u, accessToken)
    })()

    __ensureUserInflight.set(key, p)
    try {
      return await p
    } finally {
      // Keep a short-lived cache to prevent rapid remount spam.
      setTimeout(() => __ensureUserInflight.delete(key), 3000)
    }
  } catch {
    return null
  }
...
const ensurePiUserRecord = async (authUser: any, accessToken?: string): Promise<any> => {
