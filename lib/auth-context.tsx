  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      localStorage.removeItem(PI_USER_ID_STORAGE_KEY)
        ...
        log("[v0] AuthProvider: getSession result:", session ? "has session" : "no session")

        if (session?.user) {
          log("[v0] AuthProvider: Found Supabase session for user:", session.user.id)
          const email = session.user.email || undefined
					...
					log("[v0] AuthProvider: ensurePiUserRecord returned:", userData?.pi_username || "(fallback)")
						...
						setUser({
							...
							memberLabel: (userData as any)?.member_label === "trusted" ? "trusted" : "regular",
							providerLabel: (((userData as any)?.provider_label as any) || "unverified") as any,
							providerApproved: Boolean((userData as any)?.provider_approved ?? false),
						})
        }
      } catch (error) {
        ...
        setIsLoading(false)
        log("[v0] AuthProvider: initializeAuth complete, setting isLoading = false")
      }
    }

    if (hasAuthHash) {
      log("[v0] AuthProvider: Skipping init due to auth hash present")
      setIsLoading(false)
    } else {
      initializeAuth()
    }
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === "SIGNED_OUT") {
        log("[v0] User signed out, clearing state")
        setUser(null)
        sessionStorage.removeItem(PI_USER_STORAGE_KEY)
      sessionStorage.removeItem(PI_USER_ID_STORAGE_KEY)
        localStorage.removeItem(PI_USER_STORAGE_KEY)
      ...
      if (event === "SIGNED_IN" && session?.user) {
        log("[v0] User signed in, fetching user data")
	        const email = session.user.email || undefined
