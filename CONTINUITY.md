- Goal (incl. success criteria):
  ...
  - Admin view: on the same **/support** page, **only root admin (hlong295)** can add/edit/delete experts and toggle show/hide.
...
-- Done:
  ...
  - Updated **/support** page to render 3 service blocks + public expert lists + inline admin panel (visible only to root admin hlong295).
...
- Now:
  - Ship patch to admin upsert API so adding/updating experts works even when fee fields are left blank.

-- Next:
  - User runs SQL_pi_buy_sell_experts.sql on Supabase.
  - Verify on Pi Browser: admin panel appears only for root admin (hlong295), and public list shows active experts.
