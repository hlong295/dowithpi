...
- 2026-01-27: P5.1 Profile — dữ liệu thật/ổn định (không placeholder)
  - Goal:
    - Trang /profile hiển thị thông tin thật (pi_users/users) + ví PITD (pitd_wallets) và số liệu cơ bản.
    - Không đổi UI; chỉ thay dữ liệu mock bằng dữ liệu DB.
  - Key files:
    - app/profile/page.tsx (server wrapper + Suspense)
    - app/profile/profile-client.tsx (client-only UI + inline edit + useSearchParams)
    - app/api/profile/route.ts (server-only summary; resolve master user)
