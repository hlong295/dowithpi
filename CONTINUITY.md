...
- 2026-01-13: buy-sell-pi PINETFIX5: client sends auth headers to rates/editors APIs; UI shows edit block for admin fallback; force app_settings.updated_at on update.

- Now:
  - Fix Buy-Sell-Pi permissions + editor search: isAdmin computed correctly (useAuth.isAdmin is a function), admin block hidden for guests/members unless allowed; Root Admin live-search users + grant/revoke editor.
  - Fix updated_at display format to local HCM timezone and 'HH:mm DD/MM/YYYY'.
