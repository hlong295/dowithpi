...
- Update (fix46 - UI branding only):
  - Requirement: Replace top-left header logo and change displayed app name from **PITODO** → **DOWITHPI**.
  - Changes (UI only):
    - `components/header.tsx`: header logo now uses `/dowithpi-logo.png`.
    - `lib/translations.ts`: `appName` updated to `DOWITHPI` (VI/EN).
  - No changes to payments (PITD/Pi), auth flows, or PITD server-only constraints.
