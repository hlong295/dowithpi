...
- Current patch: dowithpi_perf_pibrowser_fix03_lazy_supabase_PATCH04 (fix runtime ReferenceError: ensureUserOnce is not defined; use ensurePiUserRecord for both initializeAuth and email login).

- Done:
- 2026-01-29: Pi Browser perf attempt: lazy-load supabase-js + supabase query helpers in lib/auth-context.tsx to shrink critical client bundle (no UI change).
  - 2026-01-30: Build fix (Vercel): lib/auth-context.tsx removed invalid `await` in non-async functions while keeping lazy Supabase client (promise-based setup inside useEffect + logout).
- 2026-01-30: Fix email login/runtime crash: removed missing ensureUserOnce usage in lib/auth-context.tsx; replaced with existing deduped ensurePiUserRecord to prevent auth init hanging.
  - 2026-01-30: PERF (Pi Browser): lazy-load Supabase browser client (dynamic import) in ProductCard, ReviewsModal, Category page, Product detail page, Product review page to keep supabase-js out of critical home bundle (no UI change).
  - 2026-01-30: PERF (Pi Browser): lazy-load Supabase browser client (dynamic import) in ProductCard, ReviewsModal, Category page, Product detail page, Product review page to keep supabase-js out of critical home bundle (no UI change).
  - 2026-01-30: PERF (Pi Browser): lazy-load Supabase browser client (dynamic import) in ProductCard, ReviewsModal, Category page, Product detail page, Product review page to keep supabase-js out of critical home bundle (no UI change).
  - 2026-01-30: PERF (Pi Browser): lazy-load Supabase browser client (dynamic import) in ProductCard, ReviewsModal, Category page, Product detail page, Product review page to keep supabase-js out of critical home bundle (no UI change).
  - 2026-01-30: PERF (Pi Browser): lazy-load Supabase browser client (dynamic import) in ProductCard, ReviewsModal, Category page, Product detail page, Product review page to keep supabase-js out of critical home bundle (no UI change).
 ...
 - Now:
- Validate Pi Browser load time on pinet domain; use ?dbg=1 overlay to capture timings.
  - PERF (Pi Browser load ~30s): reduce first-load overhead without changing UI.
...
- Open questions (UNCONFIRMED if needed):
  - UNCONFIRMED: Is the 30s delay mostly before any HTML shows (TTFB) or after HTML but before UI becomes interactive (JS/CSS)?

- Working set (files/ids/commands):
  - app/layout.tsx (Pi SDK loader)
  - components/pi-sdk-loader.tsx
  - components/debug-perf-overlay.tsx
