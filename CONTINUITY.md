...
- Key decisions:
  - Cho phép **admin + thành viên được root admin chỉ định** cập nhật giá mua/bán Pi.
  - Danh sách được phép cập nhật lưu tại **public.app_settings.pi_exchange_editor_ids (text[])**.
  - Root admin quản lý allowlist qua API **/api/admin/pi-exchange/editors**; UI quản trị chỉ hiển thị với root admin.
