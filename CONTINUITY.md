...
- Key decisions:
  ...
  - Tạo bảng DB riêng: public.pi_support_experts (user_id -> public.users.id) để lưu chuyên gia (category, username, phone, chat, pricing...).
...
-- Done:
  ...
  - Fixed admin upsert "MISSING_USER_ID": admin API now resolves user_id from username (pi_username) using resolveMasterUserId().
