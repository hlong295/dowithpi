- Goal (incl. success criteria):
  - Fix triệt để lỗi auth sau patch lazy supabase:
    - Vercel build PASS (không còn `await isn't allowed in non-async function`).
    - Email login (mrjqka) hoạt động trên domain Vercel: không còn lỗi `ensureUserOnce is not defined` / `log is not defined`.
    - Email login không bị gọi `/api/auth/ensure-user` sai payload (không còn 400 do `userId` undefined).
    - Pi login (hlong295) trên Pi Browser vẫn hoạt động (KHÔNG phá login).
  - Không đổi UI, không thêm hiển thị ngoài debug nhỏ (nếu cần).
...
- State:
  - Baseline (input): dowithpi_perf_pibrowser_fix03_lazy_supabase_PATCH05.zip.
  - Current patch: dowithpi_perf_pibrowser_fix03_lazy_supabase_PATCH06.zip.

- Done:
  - PATCH06: Sửa lỗi auth email login bị 400 và hiển thị user undefined do `ensureUserOnce` bị override sai signature.
    - Trước đây code gọi `ensureUserOnce(identifier, method, sessionUser)` nhưng file lại định nghĩa `ensureUserOnce(sessionUser)` → truyền string vào gây `/api/auth/ensure-user` nhận `userId` undefined (400).
    - Đã khôi phục `ensureUserOnce(identifier, method, sessionUser?)` + normalize sessionUser (hỗ trợ SupabaseUser / PiUser uid) + dedupe inflight.
  ...
  - P4.1 PITD Rules (admin-only): fixed UNAUTHORIZED on save by changing `/api/admin/pitd-rules` to authorize via `pi_users.id = userId` (from header/cookie) instead of relying on `x-pi-username`.
  - P4.1 PITD Rules load (Pi Browser): fixed client crash `ReferenceError: r is not defined` in `app/admin/pitd-management/page.tsx` so the GET `/api/admin/pitd-rules` response can populate form fields after re-opening the page.

  - PERF (Pi Browser):
    - app/layout.tsx: replace unconditional Pi SDK Script with PiSdkLoader (only loads sdk.minepi.com when window.Pi is missing).
    - next.config.mjs: enable optimizePackageImports for lucide + radix packages to reduce bundle overhead on slower webviews.
    - components/debug-perf-overlay.tsx: optional on-screen perf timings when `?dbg=1` (navigation timing + API ping), no UI change by default.
- Now:
  - User test lại trên Vercel: email login mrjqka → vào /account & /profile không bị loading/username rỗng.
  - User test lại trên Pi Browser: pi login hlong295 vẫn OK.

- 2026-01-27: P4.2 Earn — Daily missions (tasks page + admin config + claim bugfix)
  - Goal:
    - Trang /missions: nếu chưa đăng nhập → banner mời đăng nhập và ẩn danh sách nhiệm vụ.
    - Nếu đã đăng nhập → banner hiển thị PITD balance + tiến độ hoàn thành / tổng nhiệm vụ trong ngày.
    - Admin/root_admin: cấu hình nhiệm vụ trong ngày + reward PITD ngay trên trang /missions.
    - Claim nhiệm vụ: mỗi nhiệm vụ 1 lần/ngày; log vào pitd_transactions; không farm vô hạn.
  - Key files:
    - app/missions/page.tsx
    - app/api/missions/daily/route.ts
    - app/api/missions/claim/route.ts
    - app/api/missions/config/route.ts
    - lib/pitd/ledger.ts (updateWalletBalance overload)

- 2026-01-29: P5.1 Profile — bind DB data (KHÔNG đổi UI fix18)
  - Baseline input: dowithpi_29126.zip (user confirmed: Profile UI đã đúng theo fix18).
  - Goal:
    - Trang /profile dùng dữ liệu thật từ DB (pi_users/products/reviews/user_purchases) thay cho mock.
    - Không đổi UI; chỉ thay data + cho phép bật debug bằng query `?dbg=1` (Pi Browser không có console).
  - Done (this patch):
    - app/profile/page.tsx: bỏ mock profileData, fetch server API `/api/profile/me`, map vào đúng các field UI.
    - app/api/profile/me/route.ts: server-side admin đọc pi_users + stats (rating/reviews/exchanges/categories) theo provider.
  - Now:
    - User test trên Pi Browser: kiểm tra badge Provider/Uy tín/Xác thực; rating/count; exchange count; join date.
  - Next:
    - (Nếu user yêu cầu) thêm inline edit từng dòng/block nhưng vẫn giữ nguyên UI.
  - Working set:
    - app/profile/page.tsx
    - app/api/profile/me/route.ts

  - 2026-01-28: Build fix (Vercel) for /profile
    - Fix: wrap `useSearchParams()` inside a Suspense boundary by splitting /profile into server wrapper + client component.
    - Avoids: `missing-suspense-with-csr-bailout` prerender error.


- Next:
  - Nếu còn lỗi auth khác, bật debug nhỏ trên trang login/account theo `?dbg=1` (không đổi UI mặc định).

- Open questions (UNCONFIRMED if needed):
  - UNCONFIRMED: Sau PATCH06, Vercel email login có còn gọi `/api/auth/ensure-user` bị 400 không?

- Working set (files/ids/commands):
  - lib/auth-context.tsx (ensureUserOnce signature + inflight dedupe)
  - app/api/auth/ensure-user/route.ts (expects userId)
...
- 2026-01-28: P5.1 Profile (avatar + editable sections + PITD wallet block)
  - DB baseline notes:
    - provider name/description: **pi_users.provider_business_name / provider_business_description**
    - intro/provider contact fields: **profile_extras** (intro, activity_area, service_field, provider_contact, provider_hours, provider_address)
    - avatar: **public.users.avatar_url** (storage bucket: user-uploads)
  - Changes:
    - Added API: `app/api/profile/extras/route.ts` (GET/PUT; root admin can pass userId)
    - Hardened APIs (server-auth; root admin can target userId):
      - `app/api/settings/profile/route.ts`
      - `app/api/settings/avatar/route.ts` (upload to storage bucket user-uploads; updates users.avatar_url)
      - `app/api/profile/route.ts` (fix provider_business_description field; root admin target via query)
    - Updated UI: `app/profile/page.tsx`
      - Fetch real profile/users/profile_extras + PITD wallet via APIs (no mock)
      - Editable avatar + display name (self; root admin can target with query)
      - Editable About + Provider blocks; provider block only for approved provider
      - Added PITD wallet card showing balance/locked/total_spent/address
      - On-screen debug/errors for Pi Browser screenshots

- 2026-01-29: P5.1 Profile navigation (Pi Browser)
  - Fix: Bottom navbar "Profile" now routes to **/account** when not logged in (same behavior as Header avatar), and routes to **/profile** when logged in.
  - Goal: avoid slow "please login" interstitial and show login options immediately.

---

## Patch notes
- PATCH08 (this chat): Fix email/user session regressions on Vercel
  - `/api/auth/ensure-user` now supports `Authorization: Bearer <supabase access token>` and can upsert user rows via a user-scoped Supabase client (RLS), so it no longer hard-depends on `SUPABASE_SERVICE_ROLE_KEY` for normal email sessions.
  - `lib/pitd/require-user` bearer-token validation now uses anon+token (`supabase.auth.getUser()`) instead of service-role, preventing 401/500 cascades when service-role env is missing.
  - `/api/profile/me` now falls back to user-scoped Supabase client when service-role is missing, keeping Profile stable.
