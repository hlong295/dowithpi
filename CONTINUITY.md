...
- Update (fix49):
  - Vercel build warning/error: `extractUserIdFromRequest is not exported from ./require-user` (used by `lib/pitd/pi-identity.ts`).
  - Fix: export `extractUserIdFromRequest(req)` from `lib/pitd/require-user.ts` as a best-effort identity extractor (Pi header/cookies first), without hitting Supabase auth.
