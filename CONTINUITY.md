...
- State:
  - Current working patch: buildfix44 (local).
...
- Now:
  - Provide new baseline zip for retest on Pi Browser: Lottery /lucky-spin banner must show the correct “no new event” messages (hide status pill, show waiting text) when there is no active/new lottery event.

- Next:
  - If banner still shows old content, capture screenshot with `?dbg=1` so we can confirm what `/api/lottery/event` returned.
