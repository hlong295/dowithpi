...
export default function ExchangeModal({
  ...
  // Pi Browser has no console => show debug lines on-screen during PI payment.
  const [debugLines, setDebugLines] = useState<string[]>([])
  const pushDebug = (line: string) => {
    const ts = new Date().toISOString()
    setDebugLines((prev) => [...prev, `[${ts}] ${line}`].slice(-30))
  }

  const getSandboxFlag = () => {
    try {
      const qs = new URLSearchParams(window.location.search)
      if (qs.get("sandbox") === "1") return true
      if (qs.get("sandbox") === "0") return false
      const ls = window.localStorage.getItem("PI_SANDBOX")
      if (ls === "1") return true
      if (ls === "0") return false
      // best-effort hints from Pi SDK object
      const Pi: any = (window as any).Pi
      if (typeof Pi?.isSandbox === "boolean") return Pi.isSandbox
      if (typeof Pi?.sandbox === "boolean") return Pi.sandbox
    } catch {}
    return false
  }
  ...
  const handlePiPayment = async () => {
    ...
    pushDebug(`PI payment start | amount=${amountPi}`)
    pushDebug(`sandbox=${getSandboxFlag() ? "true" : "false"}`)

    const ready = await ensurePiSdkReady()
    pushDebug(`ensurePiSdkReady: ${ready.ok ? "ok" : `fail(${ready.error})`}`)
    if (!ready.ok) throw new Error(ready.error)

    // Ensure "payments" scope is granted before calling Pi.createPayment()
    try {
      pushDebug("Pi.authenticate(scopes: username,payments) ...")
      await signInWithPi(["username", "payments"])
      pushDebug("Pi.authenticate: OK")
    } catch (e: any) {
      const key = getPiSdkErrorKey(e)
      pushDebug(`Pi.authenticate: FAIL(${key}) ${e?.message || String(e)}`)
    ...
    await createPiPayment(
      ...
      },
      {
        onReadyForServerApproval: async (paymentId: string) => {
          try {
            pushDebug(`onReadyForServerApproval(paymentId=${paymentId})`)
            ...
            if (!r.ok) {
              ...
              pushDebug(`approve FAIL status=${r.status} body=${JSON.stringify(j)}`)
              setErr(`APPROVE_FAIL: ${j?.error || r.statusText || r.status}`)
            } else {
              const j = await r.json().catch(() => ({}))
              pushDebug(`approve OK status=${r.status} body=${JSON.stringify(j)}`)
            }
          } catch (e: any) {
            pushDebug(`approve EXC: ${e?.message || String(e)}`)
            setErr(`APPROVE_ERR: ${e?.message || String(e)}`)
          }
        },
        onReadyForServerCompletion: async (paymentId: string, txid: string) => {
          try {
            pushDebug(`onReadyForServerCompletion(paymentId=${paymentId}, txid=${txid})`)
            const r = await fetch("/api/payments/complete", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ paymentId, txid, sandbox: getSandboxFlag() }),
            })
            if (!r.ok) {
              const j = await r.json().catch(() => ({}))
              pushDebug(`complete FAIL status=${r.status} body=${JSON.stringify(j)}`)
              setErr(`COMPLETE_FAIL: ${j?.error || r.statusText || r.status}`)
            } else {
              const j = await r.json().catch(() => ({}))
              pushDebug(`complete OK status=${r.status} body=${JSON.stringify(j)}`)
            }
          } catch (e: any) {
            pushDebug(`complete EXC: ${e?.message || String(e)}`)
        ...
        onCancel: (paymentId: string) => {
          pushDebug(`onCancel(paymentId=${paymentId})`)
          setBusy(false)
          setErr("Payment cancelled.")
        },
        onError: (error: Error, payment?: AnyObj) => {
          pushDebug(`onError: ${String(error?.message || error)} | payment=${payment ? JSON.stringify(payment) : "null"}`)
          setBusy(false)
        ...
        <div className="p-4">
          {done ? (
            ...
            <>
              ...
              {selectedCurrency === "PI" && debugLines.length > 0 ? (
                <div className="mt-3 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2">
                  <div className="text-xs font-semibold text-gray-700">PI Payment Debug (onscreen)</div>
                  <pre className="mt-1 whitespace-pre-wrap break-words text-[11px] leading-4 text-gray-700">
                    {debugLines.join("\n")}
                  </pre>
                </div>
              ) : null}
