...
export default function ExchangeModal({
  ...
  const debugEnabled = String(process.env.NEXT_PUBLIC_PI_SANDBOX || "").toLowerCase() === "true"
  ...
  const handlePiPayment = async () => {
    ...
    if (!product) throw new Error("NO_PRODUCT")
	    ...
	    // Pi Browser may run Sandbox/Testnet or Mainnet.
	    // Pass this flag to server so it can choose the correct Pi API key.
	    const envSandbox = String(process.env.NEXT_PUBLIC_PI_SANDBOX || "").toLowerCase()
	    const sandbox = envSandbox === "1" || envSandbox === "true" || envSandbox === "yes"
      ...
      },
      {
        ...
        onReadyForServerCompletion: async (paymentId: string, txid: string) => {
          ...
          try {
              ...
              method: "POST",
              headers: { "Content-Type": "application/json" },
	              body: JSON.stringify({ paymentId, txid, sandbox }),
            })
