...
export function BottomNav() {
  ...
  // IMPORTANT (P5.1): Profile tab must behave like Header Account button.
  // If user is not logged in, route to /account (shows login options immediately).
  // If logged in, route to /profile.
  const profileHref = user ? "/profile" : "/account"
  ...
  const navItems = [
    ...
    { id: "missions", href: "/missions", icon: Target, labelKey: "navMissions" as const, isCenter: true },
    { id: "lucky-spin", href: "/lucky-spin", icon: Gift, labelKey: "navLuckySpin" as const },
    { id: "profile", href: profileHref, icon: User, labelKey: "navProfile" as const },
  ]
        ...
        <div className="grid grid-cols-5 h-16 items-end pb-1">
          {navItems.map((item) => {
            ...
            const isActive =
              ...
              // When not logged in, Profile tab routes to /account. Keep it highlighted for consistency.
              (isProfile && !user && pathname === "/account")
