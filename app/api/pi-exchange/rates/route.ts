...
async function ensureAppSettingsRow(supabase: any) {
  // If multiple rows exist, prefer the most recently updated row that actually
  // contains Pi exchange data (we have seen cases where a newer row is created/updated
  // for other settings and leaves pi_buy/sell as NULL, which would show 0 on Home).
  // Fallback to the newest row otherwise.
  try {
    const { data: withPi } = await supabase
      .from("app_settings")
      .select("*")
      .or("pi_exchange_updated_at.not.is.null,pi_buy_price_vnd.not.is.null,pi_sell_price_vnd.not.is.null")
      .order("updated_at", { ascending: false })
      .order("created_at", { ascending: false })
      .limit(1)
      .maybeSingle();
    if (withPi) return withPi;
  } catch {
    // If the DB is missing these columns, just use the generic latest-row logic below.
  }

  // Prefer the most recently updated row if multiple exist (defensive).
