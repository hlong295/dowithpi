...
async function ensureAppSettingsRow(supabase: any) {
  ...
  // If older builds accidentally inserted a non-1 row, try to reuse its values to avoid "losing" prices/editors.
  ...
  const base: any = anyRow || {};

  // Minimal defaults (match existing code patterns).
  // IMPORTANT: enforce singleton id=1 so other routes always read the same row.
  // Do NOT reference optional columns here.
  const { data: inserted, error: insErr } = await supabase
    .from("app_settings")
    .upsert(
      {
        id: 1,
        // keep any prior values if they exist
        service_fee_percentage: base.service_fee_percentage ?? 2,
        tax_percentage: base.tax_percentage ?? 8,
        pi_buy_price_vnd: base.pi_buy_price_vnd ?? null,
        pi_sell_price_vnd: base.pi_sell_price_vnd ?? null,
        pi_exchange_updated_at: base.pi_exchange_updated_at ?? null,
        pi_exchange_updated_by: base.pi_exchange_updated_by ?? null,
        pi_exchange_editor_ids: base.pi_exchange_editor_ids ?? null,
      },
      { onConflict: "id" }
    )
    .select("*")
