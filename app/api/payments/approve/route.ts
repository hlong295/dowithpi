// Pi Wallet approval has a short window.
// Use Edge runtime to minimize cold-start latency.
...
// Pi Wallet needs server approval quickly, but 3-4s is often too tight on real networks.
// Keep a small timeout, but give enough room for Pi API to respond.
...
function getPiApiKey(sandbox: boolean) {
  // Support multiple env var names to avoid mis-config on Vercel/hosting
  const raw = sandbox
    ? (process.env.PI_API_KEY_SANDBOX || process.env.PI_PLATFORM_API_KEY_SANDBOX || process.env.PI_APP_API_KEY_SANDBOX || process.env.PI_SERVER_API_KEY_SANDBOX || "")
    : (process.env.PI_API_KEY || process.env.PI_PLATFORM_API_KEY || process.env.PI_APP_API_KEY || process.env.PI_SERVER_API_KEY || process.env.PI_DEV_API_KEY || "")
  return raw.trim()
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const apiKey = getPiApiKey(sandbox)
    if (!apiKey) {
      return NextResponse.json(
        {
          error: sandbox
            ? "Missing PI_API_KEY_SANDBOX on server (set in hosting env)"
            : "Missing PI_API_KEY on server (set in hosting env)",
        },
        { status: 500 },
      )
    }
