...
// Pi Wallet expects server calls (detail/complete) to be bounded.
// Give Pi API enough time, while still preventing hanging requests.
...
function getPiApiKey(sandbox: boolean) {
  ...
  // Sandbox/Testnet and Mainnet commonly use different API keys.
  const sandboxRaw =
    process.env.PI_API_KEY_SANDBOX ||
    process.env.PI_PLATFORM_API_KEY_SANDBOX ||
    process.env.PI_APP_API_KEY_SANDBOX ||
    process.env.PI_SERVER_API_KEY_SANDBOX ||
    process.env.PI_DEV_API_KEY_SANDBOX ||
    ""
  ...
  const chosen = (sandbox ? sandboxRaw : mainnetRaw).trim()
  return chosen
}
