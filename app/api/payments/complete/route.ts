  ...
  try {
    ...
    try {
      ...
      if (existingPay?.status === "completed") {
        ...
        // Also keep a small shape for item-type inference (voucher/digital/physical)
        let providerId: string | null = null
        let productForFulfillment: any = null
        if (productId) {
          const { data: prod } = await supabaseAdmin
            .from("products")
            .select("id, provider_id, media, shipping_enabled, description")
            .eq("id", productId)
            .maybeSingle()
          providerId = (prod as any)?.provider_id || null
          productForFulfillment = prod
        }
        ...
        if (orderRow) {
          ...
          if (productId && totalAmount && !Number.isNaN(totalAmount)) {
            ...
            const itemType = productForFulfillment ? inferItemType(productForFulfillment) : "physical"
            const voucherFulfillment = itemType === "voucher" ? buildVoucherFulfillment() : null
            const purchaseMetadata = voucherFulfillment ? { fulfillment: voucherFulfillment } : null
            ...
            user_id: purchaseUserId,
              ...
              payment_method: "pi",
              status: "completed",
              ...(purchaseMetadata ? { metadata: purchaseMetadata } : {}),
            })
            if (purErr) {
              dbDebug = { ...(dbDebug || {}), step: "INSERT_USER_PURCHASES", error: purErr.message }
            } else if (voucherFulfillment) {
              // Helpful for Pi Browser (no console): surface redeem code in debug payload
              dbDebug = { ...(dbDebug || {}), voucher_redeem_code: voucherFulfillment.redeem_code }
            }
          }
