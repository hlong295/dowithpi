  ...
  try {
    ...
    try {
      ...
      const { piUserId: reqPiUserId, piUsername: reqPiUsername } =
        getPiIdentityFromRequest(request as any)
      const piUidForResolve = reqPiUserId && !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(reqPiUserId)
        ? reqPiUserId
        : undefined

      // Resolve master users.id so the Account page can see purchases consistently
      const master = await resolveMasterUserId(
        supabaseAdmin,
        userId,
        piUidForResolve,
        reqPiUsername ?? undefined,
      )
