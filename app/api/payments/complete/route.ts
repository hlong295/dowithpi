...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Resolve master users.id so the Account page can see purchases consistently
      const master = await resolveMasterUserId(supabaseAdmin, userId)
      const purchaseUserId = master.userId
      ...
      if (existingPay?.status === "completed") {
        ...
        // Find product (provider_id + item_type inference)
        ...
        if (productId) {
          const { data: prod } = await supabaseAdmin
            .from("products")
            .select("id, provider_id, name, title, media, description")
            .eq("id", productId)
            .maybeSingle()
          productRow = prod
          providerId = (prod as any)?.provider_id || null
          itemType = prod ? inferItemType(prod as any) : null
        }
        ...
        if (orderRow) {
          ...
          // P2.1: if this is a voucher item, create voucher_fulfillment (server-side redeem_code).
          if (productId && totalAmount && !Number.isNaN(totalAmount)) {
            ...
            const { data: purchaseRow, error: purErr } = await supabaseAdmin
              .from("user_purchases")
              .insert({
                // Use resolved master user id (works for Pi users where client may pass pi_uid)
                user_id: purchaseUserId,
            ...
            if (purErr) {
              ...
              // Voucher fulfillment (idempotent)
              if (itemType === "voucher" && purchaseRow?.id) {
                ...
                if (!created.ok) {
                  dbDebug = { ...(dbDebug || {}), step: "CREATE_VOUCHER_FULFILLMENT", error: created.error }
                } else {
                  dbDebug = { ...(dbDebug || {}), voucher: created.fulfillment }
                }
              }

              // P2.2 Digital fulfillment (idempotent)
              if (itemType === "digital" && purchaseRow?.id) {
                ...
                if (!created.ok) {
                  dbDebug = { ...(dbDebug || {}), step: "CREATE_DIGITAL_FULFILLMENT", error: created.error }
                } else {
                  dbDebug = { ...(dbDebug || {}), digital: created.fulfillment }
                }
              }
            }
          }
