...
function getPiApiKey(sandbox: boolean) {
  // Support multiple env var names to avoid mis-config on hosting.
  // For Sandbox/Testnet you usually need a Sandbox API key.
  const prod = (
  ...
  return (
    ...
    process.env.PI_SANDBOX_API_KEY ||
    process.env.PI_PLATFORM_SANDBOX_API_KEY ||
    prod
  )
}
...
export async function POST(request: NextRequest) {
  try {
    const { paymentId, txid, sandbox } = await request.json()
    ...
    const apiKey = getPiApiKey(Boolean(sandbox))
    if (!apiKey) {
