...
export async function POST(request: NextRequest) {
  try {
    ...
    const detailUrl = `${getPiApiBase()}/payments/${encodeURIComponent(paymentId)}`
    const detailResp = await fetch(detailUrl, {
      method: "GET",
      headers: {
        // Pi Platform requires: Authorization: Key <API_KEY>  (capital K)
    ...
    const url = `${getPiApiBase()}/payments/${encodeURIComponent(paymentId)}/complete`
