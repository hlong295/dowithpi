...
import { createVoucherFulfillment } from "@/lib/fulfillment/voucher";
import { createDigitalFulfillment } from "@/lib/fulfillment/digital";
...
export async function POST(req: Request) {
  try {
    ...
    // Load product to infer item_type (voucher/digital/physical) without changing UI.
    // Keep selection conservative to avoid selecting non-existent columns.
    const { data: productRow } = await supabaseAdmin
      .from("products")
      .select("id, name, description, media, provider_id")
      .eq("id", productId)
      .maybeSingle();
    const itemType = inferItemType(productRow || {});
    ...
    // Fetch product to infer item_type (voucher/digital/physical) without DB migration.
    const { data: productRow, error: prodErr } = await supabaseAdmin
      .from("products")
      .select("id, name, title, media, description, provider_id")
      .eq("id", productId)
      .maybeSingle();
      ...
      return json(
        {
          ...
          debug: { ...debugBase, prodErr: prodErr?.message || null },
        },
        404
      );
    }

    const itemType = inferItemType(productRow as any);
    ...
    const note = `PITODO exchange - ${body.productName || (productRow as any)?.name || "product"} (${productId})`;
    ...
    // P2.1 Voucher fulfillment: server generates redeem_code (server-only).
    let voucher: any = null;
    let voucherWarning: string | null = null;
    let digital: any = null;
    let digitalWarning: string | null = null;
    if (itemType === "voucher" && purchaseRow?.id) {
      const created = await createVoucherFulfillment({
        supabaseAdmin,
        purchaseId: purchaseRow.id,
        userId: masterUserId.userId,
        productId,
        debugTag: "pitd",
      });
      if (created.ok) {
        voucher = created.fulfillment;
      } else {
        voucherWarning = "VOUCHER_FULFILLMENT_CREATE_FAILED";
      }
    }

    // P2.2 Digital goods fulfillment: server creates access record (server-only).
    if (itemType === "digital" && purchaseRow?.id) {
      const created = await createDigitalFulfillment({
        supabaseAdmin,
        purchaseId: purchaseRow.id,
        userId: masterUserId.userId,
        productId,
      ...
      if (created.ok) {
        digital = created.fulfillment;
      } else {
        digitalWarning = "DIGITAL_FULFILLMENT_CREATE_FAILED";
      }
    }

    return json({ ok: true, result: data, masterUserId, itemType, voucher, voucherWarning, digital, digitalWarning });
  } catch (e: any) {
