...
export async function POST(request: Request) {
  try {
    // Safe debug info to confirm the server is truly using the Service Role key.
    // (Does NOT expose the full secret key.)
    const adminKeyDebug = getAdminKeyDebugInfo()
    ...
    // Canary: verify this API is truly running with Service Role and can read PITD tables.
    // This prevents confusing client-side errors when ENV is misconfigured or wrong deployment is hit.

    const { error: canaryErr } = await supabase.from("pitd_wallets").select("id").limit(1)
    if (canaryErr) {
      const detail = String((canaryErr as any).message || canaryErr)
      const isPermDenied = /permission denied/i.test(detail)
      const hintPermDenied = `DB đang REVOKE/GRANT khiến server (service_role) bị chặn đọc/ghi PITD.
...
-- 2) nếu có sequence (id tự tăng) thì cấp luôn
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO service_role;

-- 3) (khuyến nghị) cấp mặc định cho bảng/sequence tạo mới về sau
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT USAGE, SELECT ON SEQUENCES TO service_role;

Sau đó redeploy Vercel (Preview + Production) để chắc chắn env/key không bị cache.`
      return NextResponse.json(
        {
          ok: false,
          message: isPermDenied ? 'PITD_DB_PERMISSION_DENIED' : 'ADMIN_CANNOT_ACCESS_PITD_WALLETS',
          detail,
          hint: isPermDenied
            ? hintPermDenied
            : 'API /api/payments/pitd phải chạy với SUPABASE_SERVICE_ROLE_KEY đúng project Supabase (Production + Preview) và đúng deployment đang test.',
          adminKeyDebug,
        },
        { status: 500 }
      )
    ...
    const debug = getAdminKeyDebugInfo()
    ...
    // Common Supabase denial (most often: route is NOT using the real service_role key, or key/project mismatch)
    const lower = String(msg).toLowerCase()
    if (lower.includes("permission denied") || lower.includes("rls") || error?.code === "42501") {
      // Do an extra validation against this project's Auth endpoint.
      // This helps distinguish: "wrong service key project" vs "DB revoked privileges".
      const keyValidation = await validateServiceRoleKeyAgainstProject()

      const role = debug?.role
      const refFromUrl = debug?.refFromUrl
      const refFromIss = debug?.refFromIss

      const likelyWrongKey = !role || role !== "service_role"
      // Prefer the strong validator; fallback to heuristic parsing.
      const likelyWrongProject = keyValidation.accepted === false
        ? true
        : Boolean(refFromUrl && refFromIss && refFromUrl !== refFromIss)

      const primaryHint = likelyWrongKey
        ? "API đang KHÔNG dùng đúng Service Role key (role không phải service_role). Hãy copy đúng service_role key trong Supabase → Settings → API → Project API keys → service_role và set vào Vercel env SUPABASE_SERVICE_ROLE_KEY (Production + Preview), rồi redeploy."
        : likelyWrongProject
          ? "Service role key có vẻ thuộc dự án Supabase khác (refFromUrl != refFromIss). Hãy chắc chắn SUPABASE_SERVICE_ROLE_KEY được copy từ ĐÚNG project đang dùng (cùng project với SUPABASE_URL)."
          : "Nếu chắc chắn key đúng, thì DB privileges trên pitd_wallets/pitd_transactions đã bị REVOKE (service_role bị chặn thật). Hãy chạy SQL grant bên dưới trong Supabase SQL Editor (role postgres), rồi thử lại."

      return NextResponse.json(
        {
          message: msg,
          code: "PITD_DB_PERMISSION_DENIED",
          hint: primaryHint,
          sql_hint: [
            "-- Cấp quyền schema/table cho service_role (chạy bằng role postgres trong Supabase SQL Editor)",
            "GRANT USAGE ON SCHEMA public TO service_role;",
            "GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.pitd_wallets TO service_role;",
            "GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.pitd_transactions TO service_role;",
            "-- Nếu có sequence (id tự tăng) thì cấp thêm:",
            "GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO service_role;",
