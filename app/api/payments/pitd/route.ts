...
export async function POST(request: Request) {
  try {
    ...
    // Load product info from DB (do NOT trust providerId/amount from client)
    const { data: productRow, error: prodErr } = await supabase
      .from("products")
      .select(
        "id, name, provider_id, seller_id, pitd_amount, price, flash_sale_enabled, flash_sale_start_date, flash_sale_end_date, flash_sale_pitd_price",
      )
      .eq("id", String(productId))
      .maybeSingle()
    if (prodErr || !productRow) {
      return NextResponse.json({ message: "PRODUCT_NOT_FOUND" }, { status: 404 })
    }
    ...
    // Compute expected PITD unit price (supports flash sale PITD fields)
    const basePitd = Number((productRow as any).pitd_amount ?? (productRow as any).price ?? 0)
    const flashEnabled = Boolean((productRow as any).flash_sale_enabled)
    const flashStart = (productRow as any).flash_sale_start_date ? new Date((productRow as any).flash_sale_start_date) : null
    const flashEnd = (productRow as any).flash_sale_end_date ? new Date((productRow as any).flash_sale_end_date) : null
    const flashPitd = (productRow as any).flash_sale_pitd_price
    const now = new Date()
    const flashActive =
      flashEnabled &&
