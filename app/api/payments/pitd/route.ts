...
Sau đó redeploy Vercel (Preview + Production) để chắc chắn env/key không bị cache.`
    ...
    // Common Supabase denial (most often: route is NOT using service_role key, or DB privileges were revoked)
      ...
      return NextResponse.json(
        {
          ...
          hint:
            "DB đang chặn quyền (permission denied) nên service_role/route hiện tại KHÔNG đọc/ghi được pitd_wallets. Hãy chạy 1 lần SQL grant bên dưới trong Supabase SQL Editor (role postgres). Nếu đã grant mà vẫn lỗi, xem debug.keyRoleClaim để chắc chắn route đang dùng key service_role (không phải anon).",
          sql_hint: [
