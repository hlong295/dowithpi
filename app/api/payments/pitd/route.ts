...
import { buildVoucherFulfillment } from "@/lib/fulfillment";
...
export async function POST(req: Request) {
  try {
    ...
    // Load product to validate amount and infer item type (keeps client from spoofing)
    const { data: product, error: productErr } = await supabaseAdmin
      .from("products")
      .select(
        "id,name,price,currency,provider_id,metadata,shipping_enabled,is_active,is_featured"
      )
      .eq("id", productId)
      .maybeSingle();
      ...
      return json(
        {
          ...
          debug: { ...debugBase, productErr: productErr?.message || null },
        },
        404
      );
    }

    const serverAmount = Number(product.price) * quantity;
    // Keep expectedAmount as the client-provided intention, but enforce serverAmount.
    if (expectedAmount && Math.abs(serverAmount - expectedAmount * quantity) > 1e-6) {
      return json(
        {
    ...
    const note = `PITODO exchange - ${product.name || body.productName || "product"} (${productId})`;
    ...
    const voucherFulfillment = itemType === "voucher" ? buildVoucherFulfillment() : null;

    const totalPrice = serverAmount;
    ...
    if (note) purchaseInsertPayload.note = note;
    if (voucherFulfillment) {
      purchaseInsertPayload.metadata = { fulfillment: voucherFulfillment };
    }

    const { data: createdPurchase, error: purchaseErr } = await supabaseAdmin
      .from("user_purchases")
    ...
    return json({
      ...
      masterUserId,
      ...(voucherFulfillment
        ? { fulfillment: { kind: "voucher", redeem_code: voucherFulfillment.redeem_code } }
        : {}),
      ...(createdPurchase ? { purchaseId: createdPurchase.id } : {}),
    });
  } catch (e: any) {
