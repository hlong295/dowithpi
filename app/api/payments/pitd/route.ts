...
Sau đó redeploy Vercel (Preview + Production) để chắc chắn env/key không bị cache.`
    ...
    // Common Supabase/RLS denial: allow the response to carry enough info for debugging on Pi Browser.
    const lower = String(msg).toLowerCase()
    if (lower.includes("permission denied") || lower.includes("rls") || error?.code === "42501") {
      return NextResponse.json(
        {
          message: msg,
          code: "PITD_RLS_DENIED",
          hint:
            "API PITD phải dùng Service Role. Kiểm tra trên Vercel có set SUPABASE_SERVICE_ROLE_KEY đúng dự án Supabase chưa (Production + Preview) và đã redeploy chưa.",
          debug,
        },
        { status: 500 },
      )
