...
export async function GET(req: Request) {
  ...
  try {
    ...
    const isInWindow = hasWindow ? nowMs >= openMs && nowMs <= closeMs : String(current.status || "").toLowerCase() === "open";
    const eligible = isInWindow;
    const ineligible_reason = isInWindow ? null : "EVENT_NOT_OPEN";
