...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { data: event, error: evErr } = await admin
      .from("lottery_events")
      .select(
        "id,title,description,open_at,close_at,draw_at,status,reward_currency,max_participants,close_when_full,requires_pioneer,created_at"
      )
    ...
    if (!current) {
      ...
      const { data: created, error: cErr } = await admin
        ...
        .select(
          "id,title,description,open_at,close_at,draw_at,status,reward_currency,max_participants,close_when_full,requires_pioneer"
        )
        .single();
    ...
    // 3) Prizes
    const { data: prizes, error: pErr } = await admin
      .from("lottery_prizes")
