...
// (keep this route self-contained; no client-side PITD access)
...
export async function POST(req: Request) {
    ...
    return NextResponse.json(
      {
        ...
        dbg: dbg ? { where: "getSupabaseAdminClient", raw: String(e) } : undefined,
      },
      { status: 500 }
    );
  }
  ...
  const { userId: masterUserId } = await resolveMasterUserId(sb as any, requesterId);
  ...
  // NOTE (baseline-compat):
  // Some DB baselines do NOT have lottery_entries.idempotency_key.
  // We still accept an idempotency key from client, but we DO NOT write/query
  // the missing column. Uniqueness is enforced by DB constraints:
  //   - (event_id, user_id) one entry per user
  //   - (event_id, chosen_number) unique number per event
  ...
  const { data: ev, error: evErr } = await sb
    .from("lottery_events")
    .select("id,status,requires_pioneer,close_when_full,max_participants,open_at,close_at,reward_currency,meta")
    .eq("id", eventId)
    .maybeSingle();
  ...
  // Optional participation restriction configured by admin (meta.participant_auth)
  const meta: any = (ev as any)?.meta || {};
  const participantAuth = (meta.participant_auth || meta.participantAuth || "both") as "pi" | "email" | "both";
  if (participantAuth === "pi" && authKind !== "pi") {
    return json(403, { ok: false, error: "AUTH_NOT_ALLOWED", message: "Chỉ tài khoản Pi (Pioneer) được tham gia chương trình này." });
  }
  if (participantAuth === "email" && authKind === "pi") {
    return json(403, { ok: false, error: "AUTH_NOT_ALLOWED", message: "Chỉ tài khoản Email được tham gia chương trình này." });
  }

  // Validate chosen number against configured range (meta.number_min/max)
  const min = Number(meta.number_min ?? meta.numberMin ?? 0);
  const max = Number(meta.number_max ?? meta.numberMax ?? 9999);
  const rangeMin = Number.isFinite(min) ? min : 0;
  const rangeMax = Number.isFinite(max) ? max : 9999;
  if (chosenNumber < Math.min(rangeMin, rangeMax) || chosenNumber > Math.max(rangeMin, rangeMax)) {
    return json(400, { ok: false, error: "NUMBER_OUT_OF_RANGE", message: `Số phải nằm trong khoảng ${Math.min(rangeMin, rangeMax)} đến ${Math.max(rangeMin, rangeMax)}.` });
  }

  // Time gating is the primary source of truth (status can drift between patches).
  const now = Date.now();
  const openAtMs = ev.open_at ? new Date(ev.open_at as any).getTime() : Number.NEGATIVE_INFINITY;
  const closeAtMs = ev.close_at ? new Date(ev.close_at as any).getTime() : Number.POSITIVE_INFINITY;
  if (Number.isFinite(openAtMs) && now < openAtMs) return json(400, { ok: false, error: "EVENT_NOT_OPEN" });
  if (Number.isFinite(closeAtMs) && now >= closeAtMs) return json(400, { ok: false, error: "EVENT_CLOSED" });
  // If status explicitly indicates a terminal state, respect it.
  ...
  // Idempotency key is ignored for DBs without the column.
