...
// See /api/lottery/event for rationale: support both timestamptz and timestamp columns.
...
export async function POST(req: Request) {
  ...
  // Time-window based gating (robust if *_at columns are timestamp w/o tz).
  const nowMs = Date.now();
  const openMs = parseEventTimeMs((ev as any).open_at);
  const closeMs = parseEventTimeMs((ev as any).close_at);
  // Status gating: allow only when admin status is open AND current time is within [open, close).
  if (String((ev as any).status || "").toLowerCase() !== "open") {
    return json(400, { ok: false, error: "EVENT_NOT_OPEN" });
  }
  if (Number.isFinite(openMs) && nowMs < openMs) {
    return json(400, { ok: false, error: "EVENT_NOT_STARTED" });
  }
