...
// Human-friendly "mã kỳ quay" stored in lottery_events.meta.event_code.
// Format: LS-YYYYMMDD-HHMM-XXXX (XXXX = 4 hex chars)
...
export async function POST(req: Request) {
  try {
    ...
    // If admin is saving a NEW program while the currently edited event already has registrations,
    // we should create a NEW event row (new id) instead of updating the old one.
    // This ensures the banner "Đã đăng ký x/y" resets to 0 for the new program,
    // while preserving old registrations for history.
    let createdNewEventId: string | null = null;
    ...
    if (eventId) {
      ...
      if ((existingCount ?? 0) > 0) {
        prevEventId = eventId;
        // Force create a new event; do NOT reuse the old id.
        eventId = "";
      }
    }

    // Ensure every event has a stable "mã kỳ quay" for archiving.
    // - If we are creating a NEW event (eventId is empty), generate a new code.
    // - If updating an existing event, keep its existing code if present.
    ...
    if (eventId) {
      const { data: cur } = await supabase
        ...
        .eq("id", eventId)
        .maybeSingle();
      const curMeta = (cur as any)?.meta && typeof (cur as any).meta === "object" ? (cur as any).meta : null;
      const existing = curMeta?.event_code;
      if (typeof existing === "string" && existing.trim()) eventCode = existing.trim();
    }
    if (!eventCode) eventCode = generateEventCode(String(patch.draw_at));
    const nextMeta = patch.meta && typeof patch.meta === "object" ? patch.meta : {};
    // Never overwrite an existing code.
    if (!nextMeta.event_code) nextMeta.event_code = eventCode;
    // Also store the draw_at for convenience when viewing archived meta.
    if (!nextMeta.draw_at) nextMeta.draw_at = String(patch.draw_at);
    patch.meta = nextMeta;

    // If there is no current event yet (or we forced creating a new one), create one first,
    // then apply patch/prizes.
    if (!eventId) {
      const insertRow: Record<string, any> = {
        ...
        // Use derived status from patch when available (more reliable after parsing timezone)
        status: typeof patch.status === "string" ? patch.status : typeof event?.status === "string" ? event.status : "draft",
        reward_currency:
    ...
    return NextResponse.json({
      ok: true,
      event_id: eventId,
      created_new: Boolean(createdNewEventId),
      ...(createdNewEventId ? { new_event_id: createdNewEventId } : {}),
      ...(prevEventId ? { prev_event_id: prevEventId } : {}),
    });
  } catch (err: any) {
