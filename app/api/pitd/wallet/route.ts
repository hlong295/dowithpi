            ...
            if (!altWalletsErr && altWallets && altWallets.length) {
              ...
              if (pick?.id) {
                ...
                if (!movedErr && moved?.id) {
                  ...
                  const locked = Number(recovered.locked_balance ?? 0);
                  const totalSpent = Number(recovered.total_spent ?? 0);
                  const totalBalance = balance + locked;

                  return NextResponse.json({
                    wallet: { ...recovered, balance, locked_balance: locked, total_spent: totalSpent, total_balance: totalBalance },
                    dbg: dbgMode ? { ...dbgBase, legacy_recovered_from_user_id: pick.user_id, legacy_wallet_id: pick.id } : undefined,
                  });
                }
              }
            }
