...
function makePitdAddress(): string {
  // Required format (PITODO baseline): "PITD" + 20 random chars (A-Za-z0-9) => 24 total.
  const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
  const n = 20
  let out = ""
  // Prefer crypto RNG when available (Node runtime)
  try {
...
function isValidPitdAddress(addr: any): boolean {
  const s = String(addr ?? "").trim()
  if (!s) return false
  if (s.length !== 24) return false
  if (!s.startsWith("PITD")) return false
  // Only allow [A-Za-z0-9] after prefix
  return /^PITD[A-Za-z0-9]{20}$/.test(s)
}
  ...
  try {
    ...
    if (existing?.id) {
      ...
      const addrInvalid =
        ...
        // If it doesn't match PITD+20 (24 chars), treat as invalid.
        !isValidPitdAddress(addrStr)

      if (addrInvalid) {
        const newAddr = makePitdAddress()
            ...
            if (!altWalletsErr && altWallets && altWallets.length) {
              ...
              if (pick?.id) {
                ...
                if (!movedErr && moved?.id) {
                  ...
                  const addrInvalid = addrStr === "" || addrStr === "-" || addrStr === "—" || !isValidPitdAddress(addrStr)
                  if (addrInvalid) {
                    const newAddr = makePitdAddress()
