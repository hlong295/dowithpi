...
export async function POST(req: Request) {
  try {
    ...
    const roleRaw = (body as any)?.role ?? (body as any)?.targetRole ?? null;
    const requestedRole = roleRaw ? String(roleRaw) : null;
    ...
    const { data: target, error: targetErr } = await supabase
      .from("users")
      .select("id,user_role,provider_label")
      .eq("id", targetMaster)
      .maybeSingle();
    if (targetErr) {
    ...
    const nextRole = requestedRole ? requestedRole : approved ? "provider" : "redeemer";

    const updatePayload: Record<string, any> = {
      provider_approved: approved,
      provider_approved_at: approved ? new Date().toISOString() : null,
      provider_approved_by: approved ? requesterMaster : null,
      user_role: nextRole,
    };

    // When admin approves a provider for the first time, default label = unverified.
    if (approved && !target?.provider_label) {
      updatePayload.provider_label = "unverified";
    }
