...
export async function POST(req: Request) {
  try {
    ...
    // Base payload (no audit columns) to maximize compatibility with existing DB schema.
    ...
    if (!payloadBase.username) {
      return NextResponse.json({ ok: false, error: "MISSING_USERNAME" }, { status: 400 });
    }

    const id = normText(expert?.id);
    const payloadWithAudit: any = { ...payloadBase };
    if (id) payloadWithAudit.id = id;
    if (requesterId) {
      payloadWithAudit.updated_by = requesterId;
      if (!id) payloadWithAudit.created_by = requesterId;
    }
    ...
    // Try with audit columns first (if present). If the DB doesn't have those columns, fall back.
    let { data, error } = await doUpsert(payloadWithAudit);
    if (error) {
      const msg = error.message || "";
      const missingColumn =
        msg.includes('column "updated_by" does not exist') ||
        msg.includes('column "created_by" does not exist') ||
        msg.includes("updated_by") && msg.includes("does not exist") ||
        msg.includes("created_by") && msg.includes("does not exist");

      if (missingColumn) {
        // Retry without audit columns.
        const payloadNoAudit: any = { ...payloadBase };
        if (id) payloadNoAudit.id = id;
        const retry = await doUpsert(payloadNoAudit);
        data = retry.data;
        error = retry.error;
      }
    }
