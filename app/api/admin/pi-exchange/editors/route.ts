...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ ok: false, code: "FORBIDDEN", message: "Chỉ Root Admin mới có quyền quản lý phân quyền cập nhật giá." }, { status: 403 });
    }
...
export async function POST(req: Request) {
  try {
    ...
    if (error) {
    return NextResponse.json({ ok: false, code: "UPDATE_FAILED", message: "Không thể cập nhật danh sách editor." }, { status: 500 });
    }
