...
export async function GET(req: Request) {
  try {
    ...
    const results: any[] = [];
    try {
      // NOTE: Keep this query compatible with the current baseline DB schema.
      // Some deployments do NOT have a `username` column in `public.users`.
      const { data: u1, error: e1 } = await sb
        .from("users")
        .select("id,email,pi_username,user_role")
        .or(`email.ilike.%${qq}%,pi_username.ilike.%${qq}%`)
        .limit(10);
      if (!e1) (u1 || []).forEach((r: any) => results.push(r));
    } catch {}

    try {
      // Best-effort search on `public.pi_users`. Also keep schema-safe: many schemas
      // only have `pi_username` (no `username`).
      const { data: p1, error: e2 } = await sb
        .from("pi_users")
        .select("id,pi_username,user_role")
        .or(`pi_username.ilike.%${qq}%`)
        .limit(10);
      if (!e2) (p1 || []).forEach((r: any) => results.push({ ...r, email: null }));
    } catch {}
    ...
    const users = (merged || []).map((u: any) => ({
      id: String(u.id),
      label: u.pi_username || u.email || String(u.id),
      email: u.email || null,
