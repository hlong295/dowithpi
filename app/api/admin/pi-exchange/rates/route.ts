...
export async function POST(req: Request) {
  try {
    ...
    // Write to whichever column names exist in this deployment (and keep aliases in sync if multiple exist).
    const BUY_KEYS = ["pi_buy_price_vnd", "pi_buy_vnd", "pi_buy_price", "pi_buy", "buy_price_vnd", "buy_vnd"];
    const SELL_KEYS = ["pi_sell_price_vnd", "pi_sell_vnd", "pi_sell_price", "pi_sell", "sell_price_vnd", "sell_vnd"];

    for (const k of BUY_KEYS) if (s?.hasOwnProperty?.(k)) payload[k] = buy;
    for (const k of SELL_KEYS) if (s?.hasOwnProperty?.(k)) payload[k] = sell;

    // If none of the expected columns exist, fail clearly (avoid silent '...').
    if (Object.keys(payload).length === 0) {
      return NextResponse.json(
        { ok: false, code: "PI_RATES_COLUMNS_MISSING", message: "Thiếu cột giá Pi trong app_settings. Vui lòng kiểm tra schema." },
        { status: 500 }
      );
    }
    ...
    const payloadWithBy: any = {
      ...payload,
      pi_exchange_updated_by: requesterId,
    };
