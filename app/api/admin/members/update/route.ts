...
export async function POST(req: Request) {
  try {
    ...
    // Some callers include an explicit `action` string (e.g. set_member_label / set_provider_label).
    // It is optional; we treat missing/empty as "".
    ...
    // If caller sends label fields at top-level, merge them in.
    // We do NOT require `action` to be present, because some builds omit it.
    if (body?.memberLabel && !updates.memberLabel) {
      updates.memberLabel = body.memberLabel
    }
    if (body?.providerLabel && !updates.providerLabel) {
