...
export async function POST(req: Request) {
  try {
    ...
    if (!UUID_RE.test(targetUserId)) {
      return NextResponse.json({ error: "targetUserId must be UUID" }, { status: 400 })
    }
    ...
    const { data: piRow, error: piErr } = await supabase
      ...
      .update(patch)
      .eq("id", targetUserId)
      .select("id, is_banned, ban_type, banned_until, banned_reason, member_label, provider_label, provider_approved, user_role, updated_at")
      .maybeSingle()
