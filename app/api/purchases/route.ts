...
export async function GET(req: Request) {
  try {
    ...
    // Resolve master users.id. On Pi Browser, the client may send pi_users.id;
    // enrich with pi_uid/pi_username to resolve consistently.
    ...
    try {
      ...
      if (isUuid(userId)) {
        const { data: piRow } = await admin
          ...
          .maybeSingle();
        if (piRow) {
          piUid = (piRow as any).pi_uid || null;
          piUsername = (piRow as any).pi_username || null;
        }
      }
    } catch {}
