...
async function checkPiUsersTableExists() {
  ...
  try {
    ...
    // IMPORTANT:
    // Supabase/PostgREST can occasionally report a table as missing due to schema-cache delays
    // (messages like "schema cache" / "Could not find the 'pi_users'..." / even "does not exist").
    // Since you have verified `public.pi_users` exists in the database, we MUST NOT block
    // Pi login here and show a false "please run script" error.
    // If the table is *truly* missing, the downstream queries will fail anyway with clear errors.
    return true
  } catch (e) {
    // Do not block Pi login on transient errors here.
    return true
  }
}
