...
async function checkPiUsersTableExists() {
  // IMPORTANT:
  // In some environments (Pi Browser / preview deployments), querying the table may fail due to
  // permissions/policies or transient issues. We must NOT block Pi login with a misleading
  // "pi_users chưa được tạo" error when the table actually exists.
  // So this check is intentionally permissive: only return false when we are sure the table
  // is missing; otherwise return true.
  const supabase = await getSupabaseServerClient()

  try {
    const { error } = await supabase.from("pi_users").select("id").limit(1)

    if (!error) return true

    const msg = (error as any)?.message ? String((error as any).message) : ""
    if (msg.toLowerCase().includes("does not exist") || msg.toLowerCase().includes("relation") && msg.toLowerCase().includes("pi_users") && msg.toLowerCase().includes("does not exist")) {
      return false
    }

    // Any other error (RLS, permission denied, network, etc.) -> assume table exists
    return true
  } catch {
    return true
  }
...
async function findExistingPiUserByUsername(piUsername: string) {
  const supabase = await getSupabaseServerClient()
...
async function upsertPiUser(piUid: string, piUsername: string) {
  ...
  // 1) Prefer match by pi_uid (stable for most users)
  // 2) Fallback to match by pi_username (important when pi_uid was rotated / old rows were cleaned)
  //    This is critical for root admin hlong295 so we don't create a *new* identity after SQL cleanup.
  let existingUser = await findExistingPiUser(piUid)
  if (!existingUser && piUsername) {
    existingUser = await findExistingPiUserByUsername(piUsername, writeClient)
  }
  ...
  if (existingUser) {
	...
	// with the SAME id. We do NOT change any UI; we only harden server-side data consistency.
		...
		if (!targetUserId) {
			...
			// Secondary fallback: match by pi_username (case-insensitive)
			let byNameId: string | null = null
			if (!byUid?.id && piUsername) {
				const { data: byName, error: byNameErr } = await usersClient
					.from("users")
					.select("id")
					.ilike("pi_username", piUsername)
					.maybeSingle()
				if (byNameErr) {
					console.warn("[v0] users lookup by pi_username error (non-fatal):", byNameErr)
				} else if (byName?.id) {
					byNameId = byName.id
				}
			}

			targetUserId = byUid?.id || byNameId || randomUUID()
		}
    ...
    console.log("[v0] Updating existing Pi user:", existingUser.id)
		...
		// IMPORTANT:
		// - When DB was cleaned up or Pi uid rotated, we may have matched the user by pi_username.
		// - In that case existingUser.pi_uid may differ from the latest piUid coming from Pi SDK.
		// - We MUST update by primary key (id) to avoid creating a *new* identity.
