...
import { getSupabaseAdminClient, getAdminKeyDebugInfo } from "@/lib/supabase/admin"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const piUsername =
      body?.piUsername || body?.pi_username || metadata?.pi_username || metadata?.piUsername || metadata?.username
    ...
    // If the browser provides a Supabase access token (email login/session),
    // we can upsert using a user-scoped client (RLS) and avoid depending on
    // SUPABASE_SERVICE_ROLE_KEY (which may be missing on a new Vercel deploy).
    ...
    if (bearerToken) {
      ...
      if (!authErr && authedUser?.id) {
        if (isUuid(String(userId)) && String(userId) !== authedUser.id) {
          return NextResponse.json({ error: "AUTH_USER_MISMATCH" }, { status: 401 })
        }

        const effectiveUserId = authedUser.id
        const now = new Date().toISOString()
        const username = email ? String(email).split("@")[0] : (piUsername || "user")
          ...
          .upsert(
            {
              id: effectiveUserId,
              username,
              email: email || authedUser.email || null,
              updated_at: now,
            },
            { onConflict: "id" }
          )

        // Upsert public.pi_users (kept for compatibility; email users get a stable pi_uid marker)
        const emailPiUid = `EMAIL-${effectiveUserId}`
        const { data: existingPi } = await userClient.from("pi_users").select("id,user_role,pi_username,pi_uid").eq("id", effectiveUserId).maybeSingle()
        const keepRole = existingPi?.user_role || (email ? "provider" : "redeemer")
        await userClient
          .from("pi_users")
          .upsert(
            {
              id: effectiveUserId,
              pi_username: existingPi?.pi_username || username,
              pi_uid: existingPi?.pi_uid || emailPiUid,
              user_role: keepRole,
              updated_at: now,
            },
            { onConflict: "id" }
        ...
        if (debugEnabled) debug.push({ step: "bearer:ok", effectiveUserId })
        return NextResponse.json({ ok: true, userId: effectiveUserId, method: "bearer" })
      }
      if (debugEnabled) debug.push({ step: "bearer:invalid", error: authErr?.message || null })
    }

    let adminSupabase: any
    try {
      adminSupabase = getSupabaseAdminClient()
      if (debugEnabled) debug.push({ step: "admin:init:ok", adminKey: getAdminKeyDebugInfo() })
    } catch (e: any) {
      const msg = e?.message || String(e)
      if (debugEnabled) debug.push({ step: "admin:init:error", error: msg, adminKey: getAdminKeyDebugInfo() })
      return NextResponse.json(
        {
          error:
            msg === "MISSING_SUPABASE_SERVICE_ROLE_KEY"
              ? "MISSING_SUPABASE_SERVICE_ROLE_KEY (Pi Studio: paste key into lib/supabase/server-secrets.ts)"
              : msg,
          ...(debugEnabled ? { debug } : {}),
        },
        { status: 500 },
    ...
    if (isUuid(String(userId))) {
      const { data: userById, error: findError } = await adminSupabase
        .from("pi_users")
        .select("id, pi_uid, pi_username, full_name, user_role, verification_status, provider_approved, member_label, provider_label")
        .eq("id", userId)
        .maybeSingle()
