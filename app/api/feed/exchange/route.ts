...
import { inferItemType, type ItemType } from "@/lib/product-type"
...
export async function GET(req: Request) {
  ...
  // Optional: filter by item type via URL param (no UI/layout change).
  // Accepted: physical | voucher | digital
  const typeParam = (url.searchParams.get("type") || "").toLowerCase()
  const requestedType: ItemType | "" =
    typeParam === "physical" || typeParam === "ship"
      ? "physical"
      : typeParam === "voucher" || typeParam === "service"
        ? "voucher"
        : typeParam === "digital"
          ? "digital"
          : ""
  ...
  const applyType = (q: any) => {
    // For voucher/digital we can filter at DB level using jsonb contains.
    // For physical we keep DB broad (legacy products may have no marker)
    // and filter in-memory using inferItemType.
    if (!requestedType) return q
    if (requestedType === "voucher" || requestedType === "digital") {
      return q.contains("media", [{ type: "item_type", value: requestedType }])
    }
    return q
  }

  const flashQuery = applySearch(applyCategory(applyType(baseFlash)))
  const featuredQuery = applySearch(applyCategory(applyType(baseFeatured)))
  const newQuery = applySearch(applyCategory(applyType(baseNew)))
  ...
  // If type filter is present, filter in-memory as the safest fallback
  // (physical includes legacy products without a media marker).
  if (requestedType) {
    const keep = (p: any) => inferItemType(p) === requestedType
    flashData = flashData.filter(keep)
    featuredData = featuredData.filter(keep)
    newData = newData.filter(keep)
  }
