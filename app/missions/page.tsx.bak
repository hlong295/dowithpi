...
export default function MissionsPage() {
  ...
  const authHeaders = (() => {
    ...
    const piUsername = (piUser as any)?.pi_username || (piUser as any)?.username || ""
  ...
  useEffect(() => {
    // Always attempt load; API will respond logged_in=false if not authenticated.
    loadMissions()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user?.id, (piUser as any)?.uid, (piUser as any)?.pi_uid])
  ...
  const saveAdminConfig = async () => {
    try {
      ...
      const res = await fetch("/api/missions/config", {
        method: "POST",
        headers: { "content-type": "application/json", ...authHeaders },
        body: JSON.stringify({ day: adminConfig.day, missions: adminConfig.missions }),
      })
      const text = await res.text().catch(() => "")
      if (!res.ok) {
        setDebug(`PITD_MISSIONS_ADMIN_SAVE_HTTP_${res.status}: ${text}`)
        return
      }
      const data = text ? JSON.parse(text) : null
      if (data?.ok) {
      ...
      setDebug(`PITD_MISSIONS_ADMIN_SAVE_EXCEPTION: ${e?.message || String(e)}`)
    }
  }
