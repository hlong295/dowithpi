...
export default function BuySellPiPage() {
  ...
  const { user } = useAuth()
  // IMPORTANT: never rely on client-only admin flags to gate the admin panel.
  // We confirm admin status via an admin-only API call.
  const [adminAllowed, setAdminAllowed] = useState(false)
  const [adminChecked, setAdminChecked] = useState(false)
  // IMPORTANT: never trust client-side admin flags to gate the admin panel.
  // We derive admin visibility from a server-side check (admin API).
  const [adminAllowed, setAdminAllowed] = useState(false)
  const [adminChecked, setAdminChecked] = useState(false)
  ...
  async function loadAdminExperts() {
    try {
      ...
      const json = await res.json().catch(() => ({}))
      // Admin-check is derived from this admin-only endpoint.
      if (!res.ok) {
        // For non-admin/unauthenticated users: hide the admin panel quietly.
        ...
        // Any other error must NOT make the admin panel visible for normal users.
        // Only show the error to root admin (hlong295) to debug on Pi Browser.
        const isRootAdmin = (requesterUsername || "").toLowerCase() === "hlong295"
        setAdminAllowed(isRootAdmin)
        if (isRootAdmin) {
      ...
      if (json?.ok) {
        ...
        // If API returns ok=false but status 200, keep panel visible and show message
        const isRootAdmin = (requesterUsername || "").toLowerCase() === "hlong295"
        setAdminAllowed(isRootAdmin)
        if (isRootAdmin) {
          setAdminError(json?.error || json?.message || "LOAD_ADMIN_FAILED")
      ...
      // If the request fails unexpectedly, do not expose admin panel to normal users.
      setAdminAllowed(false)
      setAdminExperts([])
      // But if the user is actually admin and the request failed, surface the error.
      // We can't be 100% sure, so keep it quiet unless there is already some admin signal.
      if ((user as any)?.pi_username === "hlong295") {
        setAdminAllowed(true)
  ...
  async function refreshAll() {
    ...
    // Always attempt to check admin (the API is admin-only; 401/403 => not admin)
  ...
  useEffect(() => {
    // When auth state is ready, always attempt admin check.
    // This prevents leaking the admin panel if client state misreports isAdmin.
    if (requesterId) {
      loadAdminExperts()
    } else {
      setAdminAllowed(false)
