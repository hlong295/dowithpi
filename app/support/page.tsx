...
export default function BuySellPiPage() {
  ...
  // Only root admin (hlong295) can manage experts on the Support page.
  // Keep this check strictly local to this feature (do not change global admin logic).
  const isRootAdmin =
    isAdmin &&
    typeof requesterUsername === "string" &&
    requesterUsername.trim().toLowerCase() === "hlong295"
  ...
  const effectiveExperts = useMemo(() => {
    // Only root admin sees/manages the admin list (includes inactive) to allow toggling.
    if (isRootAdmin) return adminExperts
  ...
  }, [isRootAdmin, adminExperts, publicExperts])
  ...
  async function refreshAll() {
    ...
    if (isRootAdmin) await loadAdminExperts()
  }
  ...
  useEffect(() => {
    ...
    if (isRootAdmin && requesterId) {
      loadAdminExperts()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isRootAdmin, requesterId])

  async function saveExpert() {
    try {
      if (!isRootAdmin) {
        setAdminError("FORBIDDEN")
        return
      }
      setLoading(true)
      setAdminError(null)
        ...
        {isRootAdmin ? (
              <div className="bg-white rounded-3xl shadow-xl p-6 md:p-8">
                ...
                <div className="mt-6 text-xs text-gray-500">
                  Debug: isAdmin={String(isAdmin)} | isRootAdmin={String(isRootAdmin)} | adminExperts={adminExperts.length} | publicExperts={effectiveExperts.length}
