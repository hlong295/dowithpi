...
export default function ExchangePage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        // Build provider name map (show correct account that posted the product)
        const buildProviderNameMap = async (products: any[]) => {
          const providerIds = (products || [])
            .map((p: any) => p.provider_id || p.providerId || p.merchant_id || p.user_id)
            .filter(Boolean)
          ...
          if (providerIds.length > 0) {
            const { data: providersData } = await supabase
              ...
              .in("id", providerIds as any)
            for (const pr of providersData || []) {
              map[pr.id] = pr.provider_business_name || pr.pi_username || "PITODO"
            }
          }
          return map
        }
        ...
        if (filter === "flash") {
          ...
          const providerNameMap = await buildProviderNameMap(allData)
          setFlashSaleProducts(allData.map((p) => formatProduct(p, productRatings, providerNameMap)))
          setFeaturedProducts([])
          setNewProducts([])
          ...
          const providerNameMap = await buildProviderNameMap(allData)
          setFeaturedProducts(allData.map((p) => formatProduct(p, productRatings, providerNameMap)))
          setNewProducts([])
        } else if (filter === "new") {
          ...
          setFeaturedProducts([])
          const providerNameMap = await buildProviderNameMap(allData)
          setNewProducts(allData.map((p) => formatProduct(p, productRatings, providerNameMap)))
        } else {
          // Show all sections
          ...
          const providerNameMap = await buildProviderNameMap([...flashData, ...featuredData, ...newData])

          setFlashSaleProducts(flashData.map((p) => formatProduct(p, productRatings, providerNameMap)))
          setFeaturedProducts(featuredData.map((p) => formatProduct(p, productRatings, providerNameMap)))
          setNewProducts(newData.map((p) => formatProduct(p, productRatings, providerNameMap)))
        }
      } catch (error) {
  ...
  function formatProduct(
    p: any,
    productRatings: Record<string, { avg: number; count: number }>,
    ...
    return {
      ...
      providerName:
        p.provider_name ||
        p.merchant_username ||
        providerNameMap[p.provider_id || p.providerId || p.merchant_id || p.user_id] ||
        "PITODO",
