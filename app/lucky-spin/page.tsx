...
export default function LuckySpinPage() {
  ...
  async function saveAdminConfig() {
    try {
      ...
      const res = await fetch("/api/lottery/admin/config", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...authHeaders,
          // ensure root username always reaches server for checks
          ...(piUser?.username ? { "x-pi-username": piUser.username } : {}),
          ...(piUser?.uid ? { "x-pi-user-id": piUser.uid } : {}),
        },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        setAdminSaveMsg(`ADMIN_SAVE_FAILED: ${json?.error || res.status}`);
        return;
      }
            ...
            <div className="mt-4 grid grid-cols-1 gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="flex items-center gap-2 rounded-xl border border-purple-200 bg-white px-3 py-2">
                  <input
                    type="checkbox"
                    checked={!!adminEventDraft?.close_when_reach_limit}
                    onChange={(e) => setAdminEventDraft((prev) => (prev ? { ...prev, close_when_reach_limit: e.target.checked } : prev))}
                  />
                ...
                <label className="block">
                  ...
                  <input
                    type="number"
                    className="w-full rounded-xl border border-purple-200 bg-white px-3 py-2 text-sm"
                    value={adminEventDraft?.max_registrations ?? ""}
                    onChange={(e) =>
                      setAdminEventDraft((prev) => (prev ? { ...prev, max_registrations: e.target.value ? Number(e.target.value) : null } : prev))
                    }
                    placeholder="500"
                  />
                </label>
