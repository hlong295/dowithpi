...
export default function BuySellPiPage() {
  ...
  // Fallback auth header for admin APIs.
  // In some Pi Browser contexts, our custom cookies may not be forwarded reliably.
  // Server still enforces admin role by reading pi_users.user_role for this id.
  const authHeaders = useMemo(() => {
    const uid = (user as any)?.uid || (user as any)?.id
    return uid ? { "x-pi-user-id": String(uid) } : {}
  }, [user])
