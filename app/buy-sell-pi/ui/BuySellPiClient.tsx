...
function BuySellPiInner() {
  ...
  async function loadRates() {
    try {
      ...
      setRates(next);
      setCanEdit(Boolean((json as any)?.can_edit) || isAdmin);
      setIsRootAdmin(Boolean((json as any)?.is_root_admin));
  ...
  async function loadEditorsAllowlist() {
    try {
      setEditorMsg(null);
      const res = await fetch("/api/admin/pi-exchange/editors", { cache: "no-store" });
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json?.ok) {
        setEditorMsg(json?.message || json?.error || json?.code || "LOAD_EDITORS_FAILED");
        return;
      }
      setEditors(Array.isArray(json.editors) ? json.editors : []);
    } catch (e: any) {
      setEditorMsg(e?.message || "LOAD_EDITORS_ERROR");
    }
  }
  ...
  async function addEditor() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/pi-exchange/editors", {
        ...
        body: JSON.stringify({ action: "add", identifier: v }),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json?.ok) {
        setEditorMsg(json?.message || json?.error || json?.code || "ADD_FAILED");
        return;
      }
      setEditors(Array.isArray(json.editors) ? json.editors : []);
      setEditorInput("");
      setEditorMsg("Đã cấp quyền cập nhật giá.");
    } catch (e: any) {
      setEditorMsg(e?.message || "ADD_ERROR");
    }
  }
  ...
  async function removeEditor(id: string) {
    try {
      ...
      const res = await fetch("/api/admin/pi-exchange/editors", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "remove", identifier: id }),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json?.ok) {
        setEditorMsg(json?.message || json?.error || json?.code || "REMOVE_FAILED");
        return;
      }
      setEditors(Array.isArray(json.editors) ? json.editors : []);
      setEditorMsg("Đã gỡ quyền.");
    } catch (e: any) {
      setEditorMsg(e?.message || "REMOVE_ERROR");
    }
  }
  useEffect(() => {
    loadRates();
  ...
  useEffect(() => {
    if (isRootAdmin) loadEditorsAllowlist();
  }, [isRootAdmin]);
              ...
              <div className="bg-white border border-emerald-200 rounded-2xl p-5 max-w-2xl mx-auto text-left shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600">Chỉ admin hoặc người được cấp quyền mới thấy khối này.</p>
                  </div>
                </div>
            ...
            <div className="pt-4">
              <div className="bg-white border border-emerald-200 rounded-2xl p-5 max-w-2xl mx-auto text-left shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  <Shield className="h-5 w-5 text-emerald-700" />
                  <div>
                    <h3 className="text-lg font-bold text-emerald-900">Cấp quyền cập nhật giá</h3>
                    <p className="text-xs text-gray-600">Chỉ root admin mới thấy và quản lý danh sách này.</p>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-xs text-gray-600">Nhập user_id hoặc username/email</label>
                  <div className="flex items-stretch gap-2">
                    <Input
                      value={editorInput}
                      onChange={(e) => setEditorInput(e.target.value)}
                      placeholder="VD: mrjqka hoặc user_id..."
                      className="rounded-xl"
                    />
                    <Button onClick={addEditor} className="rounded-xl bg-emerald-600 hover:bg-emerald-700">
                      Thêm
                    </Button>
                  </div>

                  {editorMsg ? <p className="text-xs text-emerald-700">{editorMsg}</p> : null}

                  <div className="mt-3">
                    <p className="text-xs text-gray-600 mb-2">Danh sách được phép cập nhật:</p>
                    {editors?.length ? (
                      <div className="space-y-2">
                        {editors.map((u: any) => {
                          const label =
                            ...
                            u?.id;
                          return (
                            <div key={u?.id} className="flex items-center justify-between gap-3 bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2">
                              <div className="min-w-0">
                                <p className="text-sm font-semibold text-emerald-900 truncate">{label}</p>
                                <p className="text-[11px] text-gray-600 truncate">{u?.id}</p>
                              </div>
                              <Button
                                onClick={() => removeEditor(String(u?.id))}
                                variant="outline"
                      ...
                      <p className="text-xs text-gray-500">Chưa cấp quyền cho ai.</p>
                    )}
                  </div>
                </div>
              </div>
