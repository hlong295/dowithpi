...
function formatTime(ts: string | null) {
  ...
  try {
    ...
    // Desired: "HH:mm DD/MM/YYYY"
    if (hh && min && dd && mm && yy) return `${hh}:${min} ${dd}/${mm}/${yy}`;
...
function BuySellPiInner() {
  ...
  const isAdmin = typeof authIsAdmin === "function" ? !!authIsAdmin() : authIsAdmin === true;
  ...
  // IMPORTANT: permissions must be verified by server (Pi App Studio/pinet is unstable with client-side role checks)
  const canEditPrices = rates?.can_edit_prices === true;
  const canManageEditors = rates?.can_manage_editors === true;
  ...
  const [editorsDebug, setEditorsDebug] = useState<string>("");
  ...
  // Live search (type-ahead) for Root Admin when managing editor allowlist
  useEffect(() => {
  ...
  async function loadRates() {
    try {
      ...
      const next: PiRates = {
        ...
        updated_by: typeof json?.updated_by === "string" ? json.updated_by : null,
        updated_by_label: typeof json?.updated_by_label === "string" ? json.updated_by_label : null,
  ...
  async function loadEditors() {
    ...
    try {
      ...
      const list = Array.isArray(json?.editors) ? json.editors : [];
      setEditors(list);
      // Small on-screen debug (Pi Browser has no console)
      const raw = Array.isArray(json?.editor_ids) ? json.editor_ids : null;
      setEditorsDebug(
        `editors=${list.length}` + (raw ? ` | ids=${raw.join(",")}` : "") + (json?.reason ? ` | reason=${String(json.reason)}` : "")
      );
  ...
  async function searchUsers(q: string) {
    if (!canManageEditors) {
      setEditorMsg("Bạn không có quyền tìm kiếm.");
      return;
    }
    const qq = q.trim();
    if (!qq) {
  ...
  // IMPORTANT: The editor allowlist is stored in DB (app_settings.pi_exchange_editor_ids).
  // We must load it when Root Admin opens this page; otherwise after navigating away
  // and returning, component state resets and the list shows empty although DB still has data.
  useEffect(() => {
    if (!canManageEditors) return;
    loadEditors();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  async function saveRates() {
    // Only allow admin/editor users. Do not rely solely on client-side role checks;
    ...
    try {
      ...
      // For Pi-user auth we rely on cookies + x-pi-* headers.
      // For Email-user auth (Supabase Auth), include Bearer token.
	...
	// IMPORTANT: permissions must be verified by server (do not rely on client-side role checks)
	const updatedLabel = rates?.updated_at ? formatTime(rates.updated_at) : "";
    const updatedByLabel = (rates as any)?.updated_by_label
    ? String((rates as any).updated_by_label)
    : (rates as any)?.updated_by
      ? String((rates as any).updated_by)
      : "";
      ...
      <div className="max-w-4xl mx-auto px-4 pt-6">
        ...
        <Card className="bg-white rounded-3xl shadow-2xl p-6 md:p-10 text-center space-y-6">
          ...
          <div className="space-y-2 max-w-2xl mx-auto">
            ...
            <p className="text-sm md:text-base text-rose-600">Liên hệ: Mua - Bán Pi trực tiếp: 0938290578</p>
          </div>
          ...
          <div className="pt-2">
            <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 max-w-2xl mx-auto">
              ...
              {updatedByLabel ? (
                <div className="mt-1 text-center text-xs text-rose-600">Cập nhật bởi: {updatedByLabel}</div>
              ) : null}
            </div>
          </div>
            ...
            <div className="pt-4">
              <div className="bg-white/90 rounded-3xl p-6 md:p-8 border border-emerald-100 max-w-3xl mx-auto text-left shadow-sm">
                ...
                <div className="mt-5">
                  ...
                  {loadingSearch ? (
                    <p className="mt-2 text-xs text-gray-600">Đang tìm...</p>
                  ) : null}
                  {editorMsg ? (
                    <p className="mt-2 text-xs text-rose-700">{editorMsg}</p>
                  ) : null}
                  ...
                  <div className="mt-5">
                    ...
                    {editorsDebug ? (
                      <p className="text-[11px] text-gray-500 mb-2 select-text">debug: {editorsDebug}</p>
                    ) : null}
