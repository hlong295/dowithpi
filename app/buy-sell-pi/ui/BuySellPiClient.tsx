...
function BuySellPiInner() {
  ...
  async function loadRates() {
    try {
      ...
      // Root admin should see the current editor list immediately.
      if (next.can_manage_editors) {
        // fire-and-forget
        void loadEditors();
      }
  ...
  async function saveRates() {
    ...
    try {
      ...
      const res = await fetch("/api/pi-exchange/rates/update", {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  const updatedByLabel = (rates as any)?.updated_by ? String((rates as any).updated_by) : "";
