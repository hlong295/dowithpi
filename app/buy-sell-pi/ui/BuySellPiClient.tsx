...
function BuySellPiInner() {
  ...
  async function loadEditors() {
    ...
    try {
      ...
      setEditorMsg(null);
      const res = await fetch("/api/admin/pi-exchange/editors", { cache: "no-store", credentials: "include" });
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json?.ok) {
  ...
  async function searchUsers(q: string) {
    ...
    try {
      const res = await fetch(`/api/admin/pi-exchange/user-search?q=${encodeURIComponent(qq)}`, { cache: "no-store", credentials: "include" });
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json?.ok) {
  ...
  async function saveRates() {
    ...
    const canEdit = !!rates?.can_edit_prices;
  const canManageEditors = !!rates?.can_manage_editors;
