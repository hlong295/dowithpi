  ...
  useEffect(() => {
    async function fetchPiRates() {
      try {
        ...
        const res = await fetch("/api/pi-exchange/rates", {
          cache: "no-store",
        ...
        const buyRaw = json?.buy_vnd ?? json?.buy_price_vnd;
        const sellRaw = json?.sell_vnd ?? json?.sell_price_vnd;

        // Avoid displaying 0 when the API returns null/undefined (numeric -> Number(null) = 0).
        const buy = buyRaw === null || buyRaw === undefined || buyRaw === "" ? null : Number(buyRaw);
        const sell = sellRaw === null || sellRaw === undefined || sellRaw === "" ? null : Number(sellRaw);

        const buyText = typeof buy === "number" && Number.isFinite(buy) ? buy.toLocaleString("vi-VN") : "...";
        const sellText = typeof sell === "number" && Number.isFinite(sell) ? sell.toLocaleString("vi-VN") : "...";

        setPiRates({
          buyPrice: buyText,
          sellPrice: sellText,
          // API returns `updated_at` (ISO) + may include other aliases in the future.
          updatedAt:
      ...
      content: (
        <div className="mt-2 space-y-1 border-t border-emerald-100 pt-2">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-gray-500">{formatTimeVN(piRates.updatedAt)}</span>
          </div>
        </div>
      ),
    },
