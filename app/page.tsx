  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        // Resolve provider display names from pi_users to avoid showing the default "PITODO"
        const providerIds = Array.from(
          new Set(
            [...flashData, ...featuredData, ...newData]
              .map((p: any) => p?.provider_id)
              .filter(Boolean)
          )
        )

        const providerMap: Record<string, { name: string }> = {}
        if (providerIds.length > 0) {
          ...
          if (!providersError && providersData) {
            for (const pr of providersData as any[]) {
              ...
              if (pr.id && name) providerMap[pr.id] = { name }
            }
          }
        }
        ...
        const formatProduct = (p: any) => ({
          ...
          providerName: providerMap[p.provider_id]?.name || p.provider_name || p.merchant_username || "PITODO",
