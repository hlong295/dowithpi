  ...
  useEffect(() => {
    async function fetchPiRates() {
      try {
        ...
        const buy = Number(json?.buy_vnd ?? json?.buy_price_vnd);
        const sell = Number(json?.sell_vnd ?? json?.sell_price_vnd);

        // Display exactly like before: text values or fallback to "..."
        const buyText = Number.isFinite(buy) ? buy.toLocaleString("vi-VN") : "...";
        const sellText = Number.isFinite(sell) ? sell.toLocaleString("vi-VN") : "...";
