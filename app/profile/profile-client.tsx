...
export default function ProfilePage() {
  ...
  useEffect(() => {
    // Avoid useSearchParams() to prevent Next build requiring Suspense.
    if (typeof window === "undefined") return;
    const sp = new URLSearchParams(window.location.search);
    setDebugEnabled(sp.get("debug") === "1");
    setRequestedUserId(sp.get("userId"));
  }, []);
  ...
  const canEdit = useMemo(() => {
    ...
    if (!effectiveTargetUserId) return false;
    return isRootAdmin || effectiveTargetUserId === user.id;
  }, [effectiveTargetUserId, isRootAdmin, user?.id]);
  ...
  useEffect(() => {
    (async () => {
      // If not logged in, do not attempt to load profile/wallet data.
      if (!user?.id) {
      ...
      // Default target is the currently logged-in user.
      ...
      try {
        ...
        // Current logged-in user id from auth context
        setTargetUserId(user.id);
        pushDebug(`meId=${user.id}`);
      } catch (e: any) {
  ...
  // Not logged in -> show the same UX as the Account entry (guest card + link to /login).
  if (!authLoading && !user) {
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-4xl mx-auto px-4 py-6 pb-24">
          <h1 className="text-3xl font-bold text-purple-700 mb-4">Tài khoản</h1>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-5 flex items-center gap-4">
            ...
            <Link
              href="/login"
              className="inline-flex items-center justify-center px-6 py-2 rounded-md bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold"
            >
              Đăng nhập
            </Link>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6 mb-6">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            <div className="relative">
              ...
              {/* Upload button (only when logged in & allowed) */}
              {canEdit && (
                <>
                  <button
                    type="button"
