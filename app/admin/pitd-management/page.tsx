  ...
  async function loadData() {
    try {
      ...
      try {
        ...
        const rres = await fetch(`/api/admin/pitd-rules`, { headers })
        const rjson = await rres.json().catch(() => ({}))
        if (rres.ok && rjson?.ok && rjson?.rules) {
          const rules = rjson.rules as Record<string, any>
          setPitdRules({
            cost_post_product: rules.cost_post_product === null || rules.cost_post_product === undefined ? "" : String(rules.cost_post_product),
            cost_comment: rules.cost_comment === null || rules.cost_comment === undefined ? "" : String(rules.cost_comment),
            cost_review: rules.cost_review === null || rules.cost_review === undefined ? "" : String(rules.cost_review),
            cost_boost: rules.cost_boost === null || rules.cost_boost === undefined ? "" : String(rules.cost_boost),
            pi_to_pitd_rate: rules.pi_to_pitd_rate === null || rules.pi_to_pitd_rate === undefined ? "" : String(rules.pi_to_pitd_rate),
            topup_min_pi: rules.topup_min_pi === null || rules.topup_min_pi === undefined ? "" : String(rules.topup_min_pi),
            topup_max_pi: rules.topup_max_pi === null || rules.topup_max_pi === undefined ? "" : String(rules.topup_max_pi),
            transfer_fee_pitd: rules.transfer_fee_pitd === null || rules.transfer_fee_pitd === undefined ? "" : String(rules.transfer_fee_pitd),
            transfer_limit_per_day: rules.transfer_limit_per_day === null || rules.transfer_limit_per_day === undefined ? "" : String(rules.transfer_limit_per_day),
            transfer_limit_per_week: rules.transfer_limit_per_week === null || rules.transfer_limit_per_week === undefined ? "" : String(rules.transfer_limit_per_week),
            transfer_max_per_tx: rules.transfer_max_per_tx === null || rules.transfer_max_per_tx === undefined ? "" : String(rules.transfer_max_per_tx),
            transfer_tx_per_day: rules.transfer_tx_per_day === null || rules.transfer_tx_per_day === undefined ? "" : String(rules.transfer_tx_per_day),
            transfer_cooldown_hours: rules.transfer_cooldown_hours === null || rules.transfer_cooldown_hours === undefined ? "" : String(rules.transfer_cooldown_hours),
            transfer_eligibility: rules.transfer_eligibility || "",
          })
        } else {
        ...
        <Card className="mt-6">
          ...
          <CardContent>
            ...
            <div className="mt-4 flex items-center justify-between gap-3">
              <p className="text-sm text-muted-foreground">{rulesStatus}</p>
              <Button
                onClick={async () => {
                  try {
