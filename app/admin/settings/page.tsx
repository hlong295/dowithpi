...
type AppSettings = {
  ...
  // PITD receiver configs
  pitd_service_fee_receiver_user_id?: string | null
  pitd_tax_receiver_user_id?: string | null

  // PITD Rules (app_settings columns)
  pitd_user_post_fee?: number | null
  pitd_provider_post_fee?: number | null
  pitd_comment_fee?: number | null
  pitd_review_fee?: number | null
  pitd_boost_fee?: number | null
  pitd_pi_to_pitd_rate?: number | null
  pitd_topup_min?: number | null
  pitd_topup_max?: number | null

  pitd_transfer_enabled?: boolean | null
  pitd_transfer_fee?: number | null
  pitd_transfer_daily_limit?: number | null
  pitd_transfer_weekly_limit?: number | null
  pitd_transfer_max_per_tx?: number | null
  pitd_transfer_cooldown_hours?: number | null
}
...
export default function AdminSettingsPage() {
  ...
  // PITD Rules
  const [pitdUserPostFee, setPitdUserPostFee] = useState("")
  const [pitdProviderPostFee, setPitdProviderPostFee] = useState("")
  const [pitdCommentFee, setPitdCommentFee] = useState("")
  const [pitdReviewFee, setPitdReviewFee] = useState("")
  const [pitdBoostFee, setPitdBoostFee] = useState("")
  const [piToPitdRate, setPiToPitdRate] = useState("")
  const [pitdTopupMin, setPitdTopupMin] = useState("")
  const [pitdTopupMax, setPitdTopupMax] = useState("")

  const [pitdTransferEnabled, setPitdTransferEnabled] = useState(false)
  const [pitdTransferDailyLimit, setPitdTransferDailyLimit] = useState("")
  const [pitdTransferWeeklyLimit, setPitdTransferWeeklyLimit] = useState("")
  const [pitdTransferMaxPerTx, setPitdTransferMaxPerTx] = useState("")
  const [pitdTransferFee, setPitdTransferFee] = useState("")
  const [pitdTransferCooldownHours, setPitdTransferCooldownHours] = useState("")

  const [savingPitdRules, setSavingPitdRules] = useState(false)
  ...
  async function loadData() {
    try {
      ...
      if (settingsData) {
        ...
        // PITD rules (defaults to 0 when null)
        setPitdUserPostFee(String(settingsData.pitd_user_post_fee ?? 0))
        setPitdProviderPostFee(String(settingsData.pitd_provider_post_fee ?? 0))
        setPitdCommentFee(String(settingsData.pitd_comment_fee ?? 0))
        setPitdReviewFee(String(settingsData.pitd_review_fee ?? 0))
        setPitdBoostFee(String(settingsData.pitd_boost_fee ?? 0))
        setPitdPiToPitdRate(String(settingsData.pitd_pi_to_pitd_rate ?? 0))
        setPitdTopupMin(String(settingsData.pitd_topup_min ?? 0))
        setPitdTopupMax(String(settingsData.pitd_topup_max ?? 0))

        setPitdTransferEnabled(Boolean(settingsData.pitd_transfer_enabled ?? false))
        setPitdTransferFee(String(settingsData.pitd_transfer_fee ?? 0))
        setPitdTransferDailyLimit(String(settingsData.pitd_transfer_daily_limit ?? 0))
        setPitdTransferWeeklyLimit(String(settingsData.pitd_transfer_weekly_limit ?? 0))
        setPitdTransferMaxPerTx(String(settingsData.pitd_transfer_max_per_tx ?? 0))
        setPitdTransferCooldownHours(String(settingsData.pitd_transfer_cooldown_hours ?? 0))
...
async function handleViewHistory(wallet: SystemWallet) {
  ...
  const handleUpdatePitdRules = async () => {
    if (!session?.user) {
    ...
    const payload = {
      pitd_user_post_fee: toNum(pitdUserPostFee),
      pitd_provider_post_fee: toNum(pitdProviderPostFee),
      pitd_comment_fee: toNum(pitdCommentFee),
      pitd_review_fee: toNum(pitdReviewFee),
      pitd_boost_fee: toNum(pitdBoostFee),
      pitd_pi_to_pitd_rate: toNum(pitdPiToPitdRate),
      pitd_topup_min: toNum(pitdTopupMin),
      pitd_topup_max: toNum(pitdTopupMax),
      pitd_transfer_enabled: pitdTransferEnabled,
      pitd_transfer_daily_limit: toNum(pitdTransferDailyLimit),
      pitd_transfer_weekly_limit: toNum(pitdTransferWeeklyLimit),
      pitd_transfer_max_per_tx: toNum(pitdTransferMaxPerTx),
      pitd_transfer_fee: toNum(pitdTransferFee),
      pitd_transfer_cooldown_hours: toNum(pitdTransferCooldownHours),
    }

    for (const [k, v] of Object.entries(payload)) {
      if (k == "pitd_transfer_enabled") continue
      if (Number.isNaN(v as any)) {
        setError(`Giá trị không hợp lệ: ${k}`)
        return
      }
      if ((v as any) < 0) {
        setError(`Giá trị phải >= 0: ${k}`)
        return
      }
    }
    ...
    try {
      ...
      setMessage("Đã lưu PITD Rules")
    } catch (e: any) {
      console.error(e)
      setError(`Lưu PITD Rules thất bại: ${e?.message || "Unknown error"}`)
    } finally {
      setSubmitting(false)
    }
  }
...
<Button
        ...
        <Card className="p-6 bg-white">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-800">PITD Rules</h2>
              <p className="text-gray-600 text-sm">Cấu hình phí, rate và chính sách chuyển PITD (áp dụng server-side)</p>
            </div>
          </div>

          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <Label className="text-sm font-medium text-gray-700">cost_post_product (User thường)</Label>
              <Input type="number" step="0.01" value={pitdUserPostFee} onChange={(e) => setPitdUserPostFee(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label className="text-sm font-medium text-gray-700">cost_post_product (Provider đã duyệt)</Label>
              <Input type="number" step="0.01" value={pitdProviderPostFee} onChange={(e) => setPitdProviderPostFee(e.target.value)} className="mt-1" />
            </div>

            <div>
              <Label className="text-sm font-medium text-gray-700">cost_comment</Label>
              <Input type="number" step="0.01" value={pitdCommentFee} onChange={(e) => setPitdCommentFee(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label className="text-sm font-medium text-gray-700">cost_review</Label>
              <Input type="number" step="0.01" value={pitdReviewFee} onChange={(e) => setPitdReviewFee(e.target.value)} className="mt-1" />
            </div>

            <div>
              <Label className="text-sm font-medium text-gray-700">cost_boost/flash_unlock</Label>
              <Input type="number" step="0.01" value={pitdBoostFee} onChange={(e) => setPitdBoostFee(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label className="text-sm font-medium text-gray-700">rate Pi→PITD</Label>
              <Input type="number" step="0.000001" value={pitdPiToPitdRate} onChange={(e) => setPitdPiToPitdRate(e.target.value)} className="mt-1" />
            </div>

            <div>
              <Label className="text-sm font-medium text-gray-700">min topup (Pi→PITD)</Label>
              <Input type="number" step="0.01" value={pitdTopupMin} onChange={(e) => setPitdTopupMin(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label className="text-sm font-medium text-gray-700">max topup (Pi→PITD)</Label>
              <Input type="number" step="0.01" value={pitdTopupMax} onChange={(e) => setPitdTopupMax(e.target.value)} className="mt-1" />
            </div>

            <div className="flex items-center gap-3">
              <input type="checkbox" checked={pitdTransferEnabled} onChange={(e) => setPitdTransferEnabled(e.target.checked)} className="w-4 h-4" />
              <Label className="text-sm font-medium text-gray-700">Cho phép chuyển PITD (transfer_enabled)</Label>
            </div>
            <div>
              <Label className="text-sm font-medium text-gray-700">transfer fee</Label>
              <Input type="number" step="0.01" value={pitdTransferFee} onChange={(e) => setPitdTransferFee(e.target.value)} className="mt-1" />
            </div>

            <div>
              <Label className="text-sm font-medium text-gray-700">limit/day</Label>
              <Input type="number" step="0.01" value={pitdTransferDailyLimit} onChange={(e) => setPitdTransferDailyLimit(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label className="text-sm font-medium text-gray-700">limit/week</Label>
              <Input type="number" step="0.01" value={pitdTransferWeeklyLimit} onChange={(e) => setPitdTransferWeeklyLimit(e.target.value)} className="mt-1" />
            </div>

            <div>
              <Label className="text-sm font-medium text-gray-700">max / tx</Label>
              <Input type="number" step="0.01" value={pitdTransferMaxPerTx} onChange={(e) => setPitdTransferMaxPerTx(e.target.value)} className="mt-1" />
            </div>
            <div>
              <Label className="text-sm font-medium text-gray-700">cooldown (hours)</Label>
              <Input type="number" step="1" value={pitdTransferCooldownHours} onChange={(e) => setPitdTransferCooldownHours(e.target.value)} className="mt-1" />
            </div>
          </div>

          <Button onClick={handleUpdatePitdRules} disabled={submitting} className="w-full mt-6 bg-purple-600 hover:bg-purple-700 text-white">
            {submitting ? "Đang lưu..." : "Lưu PITD Rules"}
          </Button>
        </Card>
