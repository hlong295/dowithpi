...
function getRequesterId(user: any): string {
  const raw = user?.id ?? user?.userId ?? user?.uid ?? user?.piUserId ?? user?.user_id ?? ""
  if (typeof raw === "string") return raw
  if (raw && typeof raw === "object") {
    const v = raw.id ?? raw.userId ?? raw.uid ?? raw.value
  ...
  const handleSetProviderLabel = async (providerId: string, nextLabel: "unverified" | "verified" | "trusted") => {
    try {
      ...
      const res = await fetch("/api/admin/members/update", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "set_provider_label", requesterId, targetUserId: providerId, providerLabel: nextLabel }),
      })
      const json = await res.json().catch(() => ({}))
