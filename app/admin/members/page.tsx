...
export default function AdminMembersPage() {
  ...
  const loadMembers = async () => {
    setLoading(true)
    try {
      // Source of truth for account type is public.users.user_type (pi|email).
      // public.pi_users only exists for Pi accounts (to get pi_username, etc.).
      const { data: usersData, error: usersErr } = await supabase
        .from("users")
        .select(
          "id,email,full_name,user_type,user_role,verification_status,is_banned,ban_type,banned_until,banned_reason,member_label,provider_label,provider_approved,provider_verified_status,created_at",
        )
      ...
      const allIds = allUsers.map((u) => u.id).filter(Boolean)

      // Fetch pi_users for *all* ids so we can derive user_type when legacy rows are missing it.
      let piById: Record<string, { pi_username?: string; pi_uid?: string | null }> = {}
      if (allIds.length > 0) {
        ...
        ;(piRows || []).forEach((r) => {
          if (r?.id) piById[r.id] = { pi_username: r.pi_username, pi_uid: (r as any).pi_uid ?? null }
        })
      }

      const merged = allUsers.map((u) => {
        const pi = piById[u.id]
        const derivedType =
          u.user_type || (pi?.pi_uid ? "pi" : u.email ? "email" : null)
        return {
          ...u,
          user_type: derivedType,
          // For UI rendering (Pi list shows pi_username)
          pi_username: derivedType === "pi" ? pi?.pi_username || null : null,
        }
      })

      setPiMembers(merged.filter((u) => u.user_type === "pi"))
      setEmailMembers(merged.filter((u) => u.user_type === "email"))
    } catch (error) {
