...
export default function AdminMembersPage() {
  ...
  const loadMembers = async () => {
    ...
    try {
      ...
      const piIds = allUsers.filter((u) => u.user_type === "pi").map((u) => u.id)

      let piUsernamesById: Record<string, string> = {}
      if (piIds.length > 0) {
        ...
        ;(piRows || []).forEach((r) => {
          if (r?.id) piUsernamesById[r.id] = r.pi_username
        })
      }

      const merged = allUsers.map((u) => ({
        ...u,
        // For UI rendering (Pi list shows pi_username, Email list shows email)
        pi_username: u.user_type === "pi" ? piUsernamesById[u.id] : null,
      }))
