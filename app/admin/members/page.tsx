...
interface Member {
  ...
  member_label?: "normal" | "trusted" | null
...
const approveProvider = async (memberId: string, approve: boolean) => {
  ...
  const updateMemberLabel = async (memberId: string, nextLabel: "normal" | "trusted") => {
    try {
      ...
      const res = await fetch("/api/admin/members/update", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "set_member_label", requesterId, userId: memberId, memberLabel: nextLabel }),
      })
      const json = await res.json().catch(() => ({}))
  ...
  const updateProviderLabel = async (
    ...
    try {
      ...
      const res = await fetch("/api/admin/members/update", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "set_provider_label", requesterId, userId: memberId, providerLabel: nextLabel }),
      })
      const json = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(json?.error || "Failed")
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <Button
                            ...
                            className="rounded-lg bg-transparent"
                            onClick={() =>
                              updateMemberLabel(member.id, member.member_label === "trusted" ? "normal" : "trusted")
                            }
                          >
