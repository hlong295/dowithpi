          ...
          <Card className="rounded-xl shadow-sm bg-white/60">
            <CardContent className="p-4 text-center">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                        <Select
                          value={getBanSelectValue(member)}
                          onValueChange={(v) => {
                            if (v === "none") updateMember(member.id, { banType: "none" })
                            if (v === "temp7") updateMember(member.id, { banType: "temp", banDays: 7 })
                            if (v === "temp30") updateMember(member.id, { banType: "temp", banDays: 30 })
                            if (v === "perm") updateMember(member.id, { banType: "perm" })
                          }}
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="none">Đang hoạt động</SelectItem>
                            <SelectItem value="temp7">Khóa tạm 7 ngày</SelectItem>
                            <SelectItem value="temp30">Khóa tạm 30 ngày</SelectItem>
                            <SelectItem value="perm">Khóa vĩnh viễn</SelectItem>
                          </SelectContent>
                        </Select>

                        <Select
                          value={(member.member_label || "regular") as any}
                          onValueChange={(v) => updateMember(member.id, { memberLabel: v })}
                        >
                          <SelectTrigger className="h-8 w-[150px] rounded-lg text-xs">
                            <SelectValue placeholder="Label thành viên" />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="regular">Thành viên thường</SelectItem>
                            <SelectItem value="trusted">Thành viên uy tín</SelectItem>
                          </SelectContent>
                        </Select>

                        <Select
                          value={(member.provider_label || "unverified") as any}
                          onValueChange={(v) => updateMember(member.id, { providerLabel: v })}
                        >
                          <SelectTrigger className="h-8 w-[170px] rounded-lg text-xs">
                            <SelectValue placeholder="Label nhà cung cấp" />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="unverified">NCC chưa xác thực</SelectItem>
                            <SelectItem value="verified">NCC đã xác thực</SelectItem>
                            <SelectItem value="trusted">NCC uy tín</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
...
<div className="text-xs text-gray-500">Email Users</div>
  ...
  const updateMember = async (targetUserId: string, updates: any) => {
    if (!requesterAdmin) {
      setDebugMessage("❌ Chưa chọn Requester Admin")
      return
    }

    setLoading(true)
    setDebugMessage("⏳ Đang cập nhật thành viên...")
    ...
    try {
      ...
      if (!res.ok) {
        setDebugMessage(`❌ Lỗi update: ${data?.error || "Unknown"}${data?.details ? " | " + data.details : ""}`)
        return
      }

      const updated = data?.results?.pi_users || data?.results?.users
      if (updated?.id) {
        setPiMembers((prev) => prev.map((m) => (m.id === updated.id ? { ...m, ...updated } : m)))
        setEmailMembers((prev) => prev.map((m) => (m.id === updated.id ? { ...m, ...updated } : m)))
      }

      setDebugMessage("✅ Cập nhật thành công")
    } catch (err: any) {
      setDebugMessage(`❌ Exception: ${err?.message || err}`)
    } finally {
      setLoading(false)
    }
  }

  const getBanSelectValue = (m: Member) => {
    const banType = (m.ban_type || "").toLowerCase()
    if (banType === "perm") return "perm"
    if (banType === "temp") {
      // map common cases by remaining days if possible
      if (m.banned_until) {
        const ms = new Date(m.banned_until).getTime() - Date.now()
        const days = Math.ceil(ms / (24 * 60 * 60 * 1000))
        if (days >= 25) return "temp30"
        return "temp7"
      }
      return "temp7"
    }
    // fallback from is_banned
    if (m.is_banned && !m.banned_until) return "perm"
    return "none"
  }
