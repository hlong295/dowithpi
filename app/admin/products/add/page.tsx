...
export default function AddProductPage() {
  ...
  // P1.1: store product type in products.media (jsonb) using a special marker object
  // { type: "item_type", value: "physical" | "voucher" | "digital" }
  // We avoid UI changes by inferring type from the chosen category label (safe default = physical).
  const inferItemTypeFromCategoryId = (categoryId: string): ItemType => {
    const c = categories.find((x) => String(x.id) === String(categoryId))
    const labelRaw = (c ? getCategoryLabel(c) : "") || ""
    const label = String(labelRaw).toLowerCase()
    if (/(voucher|mã|code|dịch vụ|service)/i.test(label)) return "voucher"
    if (/(digital|tài liệu|ebook|file|download|link)/i.test(label)) return "digital"
    return "physical"
  }

  const withItemTypeMedia = (mediaArr: any[], itemType: string) => {
    const cleaned = Array.isArray(mediaArr)
      ? mediaArr.filter((m) => !(m && typeof m === "object" && String(m.type || "").toLowerCase() === "item_type"))
      : []
    return [{ type: "item_type", value: itemType }, ...cleaned]
  }
...
const loadCategories = async () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // P1.1: Inject a type marker into products.media (jsonb).
      // No UI changes: infer from chosen category label.
      ...
      const finalMedia = [
        { type: "item_type", value: inferredType, display_order: -1 },
        ...mediaWithoutType,
      ]
      ...
      const productData = {
        ...
        media: finalMedia, // Contains item_type marker + real Supabase URLs
        ...
        // PITD payment mode: if disabled, show direct contact instead of PITD exchange
      ...
      if (!res.ok || !json?.ok) {
        ...
        throw new Error(`Lỗi tạo sản phẩm: ${code}${detail}`)
      }

      console.log("[v0] Product created successfully:", json.product)
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setFormData((prev) => {
                      const next = { ...prev, categoryId: nextCategoryId }
                      if (!typeTouched) {
                        next.itemType = inferItemTypeFromCategoryId(nextCategoryId)
                      }
                      return next
                    })
                  }}
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    setTypeTouched(true)
                    setFormData({ ...formData, itemType: e.target.value as ItemType })
                  }}
                  required
                  className="w-full rounded-xl border border-gray-200 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
                >
                  <option value="physical">Physical (ship)</option>
                  <option value="voucher">Service/Voucher (redeem)</option>
                  <option value="digital">Digital (giao ngay)</option>
                </select>
              </div>
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="p-4 rounded-xl bg-white/80 space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-medium text-purple-600">Thanh toán bằng PITD</h4>
                  <Switch
                    checked={!!formData.pitdPaymentEnabled}
                    onCheckedChange={(checked) =>
                      setFormData({
                        ...formData,
                        pitdPaymentEnabled: checked,
                      })
                    }
                  />
                </div>
                ...
                {formData.pitdPaymentEnabled ? (
                  ...
                  <div className="space-y-3">
                    <p className="text-xs text-gray-600">
                      Nếu tắt thanh toán PITD, sản phẩm sẽ hiển thị thông tin liên hệ để người mua trao đổi trực tiếp.
                    </p>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label htmlFor="directContactName">Tên liên hệ</Label>
                        <Input
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="directContactPhone">Số điện thoại</Label>
                        <Input
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="directContactAddress">Địa chỉ</Label>
                      <Input
                        id="directContactAddress"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="directContactNote">Ghi chú thêm</Label>
                      <Textarea
                        id="directContactNote"
