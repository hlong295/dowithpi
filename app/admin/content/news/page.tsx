...
export default function AdminNewsCmsPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      setItems(await fetchPosts("news", q));
    } catch (e: any) {
    ...
    <AdminShell title="Tin tức CMS">
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Button asChild variant="default"><Link href="/admin/content/news/new">+ New</Link></Button>
        </div>
          ...
          <AdminJsonTable
            ...
            rowActions={(row) => (
              <Button asChild size="sm" variant="outline">
                <Link href={`/admin/content/news/${row.id}`}>Edit</Link>
              </Button>
            )}
          />
        )}
