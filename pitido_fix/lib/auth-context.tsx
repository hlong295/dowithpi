...
function setCookie(name: string, value: string, days = 30) {
  ...
  // Pi Browser is an embedded WebView; SameSite=Lax is the most reliable default.
  const attrs = [`Path=/`, `Max-Age=${maxAge}`, `SameSite=Lax`]
  if (secure) attrs.push(`Secure`)
  document.cookie = `${name}=${encodeURIComponent(value)}; ${attrs.join("; ")}`
}

function syncPiCookies(piUser: any) {
  try {
    const uid = piUser?.uid || piUser?.userId || piUser?.id
    if (!uid) return
    // Used by server PITD routes
    setCookie("pi_user_id", String(uid))
    // A compact JSON cookie consumed by server routes
    const compact = {
      userId: String(uid),
      username: piUser?.username || piUser?.pi_username || "",
      type: "pi",
    }
    setCookie("pitodo_pi_user", JSON.stringify(compact))
  } catch {
    // no-op
  }
}

function syncEmailCookies(userId: string) {
  try {
    if (!userId) return
    setCookie("pitodo_user_id", userId)
  } catch {
    // no-op
  }
}

function syncClientAuthCookies(user: AuthUser) {
  try {
    // Keep server API routes working even when cookies were not set by a callback route.
    if ((user as any)?.type === "pi") {
      const uid = (user as any)?.userId || (user as any)?.id || "";
      if (uid) {
        // Used by server PITD routes
        setCookie("pi_user_id", uid, 30);
        setCookie(
          "pitodo_pi_user",
          JSON.stringify({ userId: uid, type: "pi", piUsername: (user as any)?.piUsername || (user as any)?.username || "" }),
          30
        );
      }
    } else if ((user as any)?.type === "email") {
      const uid = (user as any)?.id || "";
      if (uid) {
        // Optional helper for server routes in environments that strip custom headers.
        setCookie("pitodo_user_id", uid, 30);
      }
    }
  } catch {
    // ignore
      ...
      if (piUserIdCookie) {
        try {
          ...
          if (response.ok) {
            ...
            deleteCookie("pitodo_pi_user_id")
            deleteCookie("pitodo_user_id")
          }
        } catch (error) {
    ...
    const initializeAuth = async () => {
      try {
          ...
          if (storedPiUserStr) {
            try {
              ...
              if (parsedPiUser && (parsedPiUser as any).type === "pi" && parsedPiUser.uid) {
                ...
                // Keep a consistent PI user id key for pages/components that read it directly.
                // This does not change the login flow; it only syncs identifiers for server APIs.
                try {
                  localStorage.setItem("pitodo_pi_user_id", parsedPiUser.uid)
                  localStorage.setItem(PI_USER_ID_STORAGE_KEY, parsedPiUser.uid)
                } catch {}

                // Also write cookies so server API routes (wallet/transactions/transfer) can authenticate on Pi Browser.
                syncClientAuthCookies(parsedPiUser)
        ...
        if (session?.user) {
          ...
          // Help server routes authenticate even if custom headers are stripped in embedded contexts.
          syncClientAuthCookies(normalizedEmailUser)
        }
      } catch (error) {
