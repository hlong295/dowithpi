...
export async function getAuthenticatedUserId(req: Request) {
  ...
  // 2) Cookie-based auth (preferred)
  const cookieHeader = req.headers.get("cookie");
  const piUser =
    parseCookieValue(cookieHeader, "pitodo_pi_user") ||
    parseCookieValue(cookieHeader, "pi_user_id") ||
    // Email/user fallback cookie (set client-side in auth-context for Pi Browser)
    parseCookieValue(cookieHeader, "pitodo_user_id") ||
    // Legacy / experimental keys
    parseCookieValue(cookieHeader, "pitodo_pi_user_id");
  ...
  const hUser = req.headers.get("x-user-id") || req.headers.get("x-pitodo-user-id");
